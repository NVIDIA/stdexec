              ...
              std::string_view method)
{
  ...
  std::size_t const cells         = accessor.cells;
  std::size_t const grid_blocks   = (cells + block_threads - 1) / block_threads;
  ...
  report_performance(grid.cells,
                     ...
                     [&]()
                     {
                       for (std::size_t compute_step = 0; compute_step < n_iterations;
                            compute_step++)
                       {
                         for_each_kernel<block_threads>
                           <<<grid_blocks, block_threads, 0, stream>>>(cells, h_updater);
                         for_each_kernel<block_threads>
                           <<<grid_blocks, block_threads, 0, stream>>>(cells, e_updater);
                       }
                       writer(false);
                       STDEXEC_TRY_CUDA_API(cudaStreamSynchronize(stream));
                     });
