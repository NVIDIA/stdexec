  ...
  struct repeat_n_sender_t {
    ...
    friend auto tag_invoke(ex::connect_t, Self&& self, Receiver r)
      ...
      return repeat_n_detail::operation_state_t<SenderId, Closure, ex::__id<Receiver>>(
        static_cast<Sender&&>(self.sender_),
        self.data_.closure_,
    ...
    template <class Self, class Fun>
    static auto apply(Self&& self, Fun&& fun) -> decltype(auto) {
      return fun(repeat_n_t{}, static_cast<Self&&>(self).data_, static_cast<Self&&>(self).sender_);
    }
...
namespace nvexec::_strm {
  namespace repeat_n {
    ...
    struct operation_state_t : operation_state_base_t<ReceiverId> {
      ...
      void start() & noexcept {
        if (this->stream_provider_.status_ != cudaSuccess) {
          ...
          this
            ->propagate_completion_signal(ex::set_error, std::move(this->stream_provider_.status_));
        } else {
          if (n_) {
            ex::start(*pred_op_state_);
          } else {
    ...
    struct sender_t {
      using __t = sender_t;
      using __id = sender_t;
      using Sender = ex::__cvref_t<SenderId>;
      using sender_concept = ex::sender_t;
  ...
  template <class Env>
  struct transform_sender_for<repeat_n_t, Env> {
    template <class Data, class Sender>
    auto operator()(ex::__ignore, Data&& data, Sender&& sndr) const {
      auto& [closure, n] = data;
      using closure_t = decltype(closure);

      return repeat_n::sender_t<ex::__cvref_id<Sender>, closure_t>(
        static_cast<Sender&&>(sndr), ex::__forward_like<Data>(closure), n);
    }
