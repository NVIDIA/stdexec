...
struct grid_t
{
  ...
  {
    return {.dx       = height / static_cast<float>(n),
...
{
  float const os2 = object_size * object_size;
...
struct grid_initializer_t
{
  ...
  {
    std::size_t const row    = cell_id / accessor.n;
    std::size_t const column = cell_id % accessor.n;
    ...
    float const soil_y      = accessor.width / 2.2f;
    float const object_y    = soil_y - 22.0f;
...
struct h_field_calculator_t
{
  ...
  STDEXEC_ATTRIBUTE(always_inline, host, device) void operator()(std::size_t cell_id) const
  {
    ...
    std::size_t const column       = cell_id % N;
    std::size_t const row          = cell_id / N;
...
struct e_field_calculator_t
{
  ...
  STDEXEC_ATTRIBUTE(always_inline, host, device) void operator()(std::size_t cell_id) const
  {
    ...
    std::size_t const column       = cell_id % N;
    std::size_t const row          = cell_id / N;
    bool const        source_owner = cell_id == source_position;
...
class result_dumper_t
{
  ...
  void write_vtk(std::string const &filename) const
  {
    ...
    for (std::size_t cell_id = 0; cell_id < accessor_.cells; cell_id++)
    {
      std::size_t const point_offset = cell_id * 4;
      fprintf(f,
              "4 %d %d %d %d\n",
              (int) (point_offset + 0),
              (int) (point_offset + 1),
...
void report_performance(std::size_t      cells,
                        std::size_t      iterations,
                        std::string_view method,
                        double           elapsed)
{
  // Assume perfect locality
  std::size_t const memory_accesses_per_cell = 6ul * 2;  // 8 + 9;
  std::size_t const memory_accesses          = iterations * cells * memory_accesses_per_cell;
  std::size_t const bytes_accessed           = memory_accesses * sizeof(float);

  double const bytes_per_second  = static_cast<double>(bytes_accessed) / elapsed;
  double const gbytes_per_second = bytes_per_second / 1024 / 1024 / 1024;
...
[[nodiscard]]
auto value(std::map<std::string_view, std::size_t> const &params,
