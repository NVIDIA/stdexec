...
template <class T1, class T2> class manual_alternative {
 public:
  manual_alternative(manual_alternative&& other) { emplace<0>(std::move(other.storage_.first)); }

  template <class... Args> explicit constexpr manual_alternative(std::in_place_t, Args&&... args) {
    emplace<0>(std::forward<Args>(args)...);
  }

  template <std::size_t Idx, class... Args> constexpr void emplace(Args&&... args) {
    if constexpr (Idx == 0) {
  ...
  template <std::size_t Idx> constexpr void destroy() noexcept {
    if constexpr (Idx == 0) {
      storage_.first.~T1();
    } else {
      storage_.second.~T2();
  ...
  template <std::size_t Idx, class Self> constexpr auto&& get(this Self&& self) noexcept {
    if constexpr (Idx == 0) {
...
template <auto> class intrusive_queue;

template <class Tp, Tp* Tp::* Next> class intrusive_queue<Next> {
 public:
...
template <class Result> struct local_operation_result {
...
template <class Result, class ErrorVariant, class Receiver> struct operation_base {
  struct stop_callback_t {
    operation_base& op_;

    void operator()() noexcept { op_.notify_stopped(); }
  };
  ...
  template <class Error> void notify_error(Error&& err) noexcept {
...
using local_env_t =
    exec::make_env_t<stdexec::env_of_t<Receiver>,
                     exec::with_t<stdexec::get_stop_token_t, stdexec::inplace_stop_token>>;

template <class Result, class ErrorVariant, class Receiver> struct local_receiver {
  ...
  template <class... Args> void set_value(Args&&... result) && noexcept {
  ...
  void set_stopped() && noexcept { local_op_->parent_.notify_stopped(); }

  local_operation_base<Result, ErrorVariant, Receiver>* local_op_;
};

template <class... Ts> using nullable_std_variant = std::variant<unit, Ts...>;

template <class... Ts>
using nullable_std_variant_for =
    stdexec::__minvoke<stdexec::__munique<stdexec::__q<nullable_std_variant>>, std::exception_ptr,
                       std::remove_cvref_t<Ts>...>;

template <class Sender, class Environment> struct traits {
...
struct local_operation
    : local_operation_base<typename traits<Sender, stdexec::env_of_t<Receiver>>::Result,
  ...
  local_operation(operation_base<Result, ErrorVariant, Receiver>& parent, Sender&& sndr) noexcept
      ...
      , child_op_(stdexec::connect(std::forward<Sender>(sndr),
                                   local_receiver<Result, ErrorVariant, Receiver>(this))) {}

  void start() noexcept { stdexec::start(child_op_); }
...
class operation
    : public operation_base<typename traits<std::ranges::range_reference_t<Range>,
                                            stdexec::env_of_t<Receiver>>::Result,
                            typename traits<std::ranges::range_reference_t<Range>,
                                            stdexec::env_of_t<Receiver>>::ErrorVariant,
                            Receiver> {
 public:
  ...
  using ErrorVariant = typename traits<std::ranges::range_reference_t<Range>,
                                       stdexec::env_of_t<Receiver>>::ErrorVariant;
    ...
    for (auto& variant : children_) {
      try {
        ...
        variant.template emplace_from<1>(
            [&] { return local_operation<Sender, Receiver>(*this, std::move(sndr)); }); // NOLINT
      } catch (...) {
        std::size_t i = 0;
        for (; i < counter; ++i) {
...
template <class Range> class sender {
 public:
  ...
  template <class Self, class Receiver>
    requires stdexec::__single_value_sender<std::ranges::range_reference_t<Range>,
  ...
  auto get_completion_signatures(Env&&) const noexcept
      -> stdexec::transform_completion_signatures_of<
          std::ranges::range_value_t<Range>, Env,
