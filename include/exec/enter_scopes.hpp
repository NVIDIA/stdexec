...
struct t {
  ...
  //  This moves the senders from the children section to the data section so we
  //  can connect/start/etc. them ourselves
        ...
        Sender>)
  {
    return ::STDEXEC::__apply(
      [](::STDEXEC::__ignore, ::STDEXEC::__ignore, auto&&... senders)
        noexcept(
          (std::is_nothrow_constructible_v<
            std::remove_cvref_t<decltype(senders)>,
            decltype(senders)> && ...))
      {
        return ::STDEXEC::__make_sexpr<tag>(
          //  TODO: Transform all the children?
          std::tuple((decltype(senders)&&)senders...));
      },
      (Sender&&)sender);
  }
};
...
struct completions;
template<typename Self, typename... Children, typename Env>
struct completions<Self, std::tuple<Children...>, Env> {
  using type = ::STDEXEC::transform_completion_signatures<
    ::STDEXEC::completion_signatures<
      ::STDEXEC::set_value_t(
        exit_sender_t<
          ::exec::exit_scope_sender_of_t<
            ::exec::like_t<Self, Children>,
            Env>...>)>,
    typename storage_for_completion_signatures<Env>::
      template __f<::exec::like_t<Self, Children>...>::completion_signatures>;
};

//  TODO: We should be able to get a better "message" than this
struct FAILED_TO_FORM_COMPLETION_SIGNATURES {};

template<typename Tuple, typename Receiver, typename = std::remove_cvref_t<Tuple>>
struct get_state_result;
template<typename Tuple, typename Receiver, typename... Senders>
struct get_state_result<Tuple, Receiver, std::tuple<Senders...>> {
  using type = state<
    Receiver,
    ::exec::like_t<Tuple, Senders>...>;
};

template<typename Tuple, typename, typename = std::remove_cvref_t<Tuple>>
struct nothrow_get_state;
template<typename Tuple, typename Receiver, typename... Senders>
struct nothrow_get_state<Tuple, Receiver, std::tuple<Senders...>> {
        ...
        -> get_state_result<::STDEXEC::__data_of<Sender>, Receiver>::type
  {
    ...
    return std::apply(
      [&](auto&&... senders) noexcept(
        nothrow_get_state<::STDEXEC::__data_of<Sender>, Receiver>::value)
      {
        return typename get_state_result<
...
template<>
struct __sexpr_impl<::exec::detail::enter_scopes::tag>
  : ::exec::detail::enter_scopes::impl {};

template<>
struct __sexpr_impl<::exec::enter_scopes_t> : ::STDEXEC::__sexpr_defaults {
