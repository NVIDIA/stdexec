...
namespace exec {
  ...
  namespace __sequence_sndr {
    ...
    using __tfx_sequence_t = transform_sender_result_t<
      stdexec::__detail::__completing_domain<set_value_t, _Sequence, _Env>,
      transform_sender_result_t<
        stdexec::__detail::__starting_domain<_Env, set_value_t>,
        _Sequence,
        _Env
      >,
      _Env
  ...
  namespace __sequence_sndr {
    ...
    struct subscribe_t {
      ...
      template <class _Sequence, class _Receiver>
      using __tfx_sequence_t = __sequence_sndr::__tfx_sequence_t<_Sequence, env_of_t<_Receiver>>;
      ...
      static constexpr auto __get_declfn() noexcept {
        ...
        if constexpr (__next_connectable<__tfx_sequence_t, _Receiver>) {
          ...
          constexpr bool __nothrow = __nothrow_connectable<
            next_sender_of_t<_Receiver, __tfx_sequence_t>,
            __stopped_means_break_t<_Receiver>
          >;
          return __declfn<__result_t, __nothrow>();
          ...
          __check_operation_state<__result_t>();
          constexpr bool __nothrow = __nothrow_tfx_sequence
      ...
      auto operator()(_Sequence&& __sequence, _Receiver&& __rcvr) const
        ...
        auto __completing_dom =
          stdexec::get_completion_domain<stdexec::set_value_t>(get_env(__sequence), __env);

        // 2. Starting domain transformation (where the operation state starts)
        auto __starting_dom = stdexec::get_domain(__env);

        auto&& __completing_tfx =
          stdexec::transform_sender(__completing_dom, static_cast<_Sequence&&>(__sequence), __env);
        auto&& __tfx_sequence = stdexec::transform_sender(
          __starting_dom, static_cast<decltype(__completing_tfx)&&>(__completing_tfx), __env);
