...
class make_sender_impl;

template<typename Tuple, typename F, typename... Objects>
class make_sender_impl<Tuple, std::tuple<F, Objects...>> {
...
public:
  ...
  {
    return std::apply(
      [&](auto&& f, auto&&... objects) noexcept(nothrow) {
        return std::apply(
...
constexpr ::STDEXEC::sender auto make_sender(
  Tuple&& tuple,
  std::tuple<storage_for_object<Ts>...>& storage) noexcept(
    noexcept(
      make_sender_impl<Tuple>::impl(
        (Tuple&&)tuple,
        storage)))
{
  return make_sender_impl<Tuple>::impl((Tuple&&)tuple, storage);
}

template<typename Tuple, typename Receiver>
class state {
  using impl_ = make_sender_impl<Tuple>;
...
class impl : public ::STDEXEC::__sexpr_defaults {
  ...
  using completions_ = ::STDEXEC::completion_signatures_of_t<
    typename make_sender_impl<::STDEXEC::__data_of<Self>>::sender_type,
    Env...>;
public:
  template<typename Self, typename... Env>
