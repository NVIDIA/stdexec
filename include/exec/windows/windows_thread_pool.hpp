...
class windows_thread_pool {
  ...
  template <class Rcvr> struct _schedule_op {
    class type;
  };
  template <class Rcvr> using schedule_op = typename _schedule_op<Rcvr>::type;

  template <class StopToken> struct _cancellable_schedule_op_base {
    class type;
    using __t = type;
  };
  ...
  template <class Rcvr> struct _cancellable_schedule_op {
    class type;
    using __t = type;
  };
  template <class Rcvr>
  ...
  template <class StopToken> struct _time_schedule_op {
    class type;
    using __t = type;
  };
  template <class StopToken>
  ...
  template <class Rcvr> struct _schedule_at_op {
    class type;
    using __t = type;
  };
  template <class Rcvr>
  ...
  template <class Duration, class Rcvr> struct _schedule_after_op {
    class type;
    using __t = type;
  };
  ...
  template <class Duration> struct _schedule_after {
    class sender;
    using __t = sender;
  };
...
public:
  ...
  explicit windows_thread_pool(std::uint32_t minThreadCount,
...
public:
  explicit type(windows_thread_pool &pool, Rcvr rcvr)
      : schedule_op_base(pool, &work_callback), rcvr_(std::move(rcvr)) {}

private:
  static void CALLBACK work_callback(PTP_CALLBACK_INSTANCE, void *workContext,
                                     PTP_WORK) noexcept {
...
protected:
  explicit type(windows_thread_pool &pool, bool isStopPossible) {
    ...
    work_ = ::CreateThreadpoolWork(isStopPossible ? &stoppable_work_callback
                                   ...
                                   static_cast<void *>(this), &environ_);
    if (work_ == nullptr) {
      DWORD errorCode = ::GetLastError();
      ::DestroyThreadpoolEnvironment(&environ_);
      throw std::system_error{static_cast<int>(errorCode),
                              std::system_category(), "CreateThreadpoolWork()"};
    }
  ...
  void request_stop() noexcept {
    ...
    do {
      ...
      if ((prevState & starting_flag) != 0) {
        ...
        prevState, prevState | stop_requested_flag, std::memory_order_acq_rel,
...
public:
  ...
  [[nodiscard]] auto get_env() const noexcept -> attrs { return attrs{pool_}; }
...
protected:
  explicit type(windows_thread_pool &pool, bool isStopPossible) {
    ...
    timer_ = ::CreateThreadpoolTimer(isStopPossible ? &stoppable_timer_callback
                                                    : &timer_callback,
                                     static_cast<void *>(this), &environ_);
    if (timer_ == nullptr) {
      DWORD errorCode = ::GetLastError();
...
private:
  ...
  static void CALLBACK timer_callback(
      [[maybe_unused]] PTP_CALLBACK_INSTANCE instance, void *timerContext,
...
class windows_thread_pool::_schedule_at_op<Rcvr>::type final
    : public windows_thread_pool::time_schedule_op<
          stdexec::stop_token_of_t<stdexec::env_of_t<Rcvr>>> {
  using base = windows_thread_pool::time_schedule_op<
      stdexec::stop_token_of_t<stdexec::env_of_t<Rcvr>>>;
...
public:
  ...
  explicit type(windows_thread_pool &pool,
                windows_thread_pool::clock_type::time_point dueTime, Rcvr rcvr)
      : base(pool, stdexec::get_stop_token(rcvr).stop_possible()),
        dueTime_(dueTime), rcvr_(std::move(rcvr)) {}
...
public:
  ...
  explicit schedule_at_sender(windows_thread_pool &pool,
                              filetime_clock::time_point dueTime)
      : pool_(&pool), dueTime_(dueTime) {}
...
public:
  ...
  explicit type(windows_thread_pool &pool, Duration duration, Rcvr rcvr)
      : base(pool, stdexec::get_stop_token(rcvr).stop_possible()),
        duration_(duration), rcvr_(std::move(rcvr)) {}
...
public:
  ...
  auto connect(Rcvr rcvr) const -> schedule_after_op<Duration, Rcvr> {
    return schedule_after_op<Duration, Rcvr>{*pool_, duration_,
...
public:
  ...
  [[nodiscard]] STDEXEC_MEMFN_DECL(auto now)(this scheduler) noexcept
      -> time_point {
    return filetime_clock::now();
  }

  [[nodiscard]] STDEXEC_MEMFN_DECL(auto schedule_at)(this scheduler self,
                                                     time_point tp) noexcept
      -> schedule_at_sender {
    return self.make_schedule_at_sender(tp);
  }

  template <class Duration>
  [[nodiscard]] STDEXEC_MEMFN_DECL(auto schedule_after)(
      this scheduler self,
      Duration d) noexcept(std::is_nothrow_move_constructible_v<Duration>)
      -> schedule_after_sender<Duration> {
    return self.make_schedule_after_sender(std::move(d));
  }
...
private:
  ...
  [[nodiscard]] auto make_schedule_at_sender(time_point tp) const noexcept
      -> schedule_at_sender {
    return schedule_at_sender{*pool_, tp};
  }

  template <class Duration>
  [[nodiscard]] auto make_schedule_after_sender(Duration d) const noexcept
      -> schedule_after_sender<Duration> {
...
inline auto windows_thread_pool::get_scheduler() noexcept
