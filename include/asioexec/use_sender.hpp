 ...
 * - Leaving advanced work to users.
 */
...
namespace asioexec {
    ...
    public:
        ...
        sender(auto &&init_start_func, auto &&...init_start_args)
            : initiation(new asio_initiation(std::forward<decltype(init_start_func)>(init_start_func), std::tuple(std::forward<decltype(init_start_args)>(init_start_args)...))) {}
        ...
        struct operation_state {
            enum class state_type { constructed, emplaced, initiated, stopped };
            ...
            using stop_callback = typename stdexec::stop_token_of_t<stdexec::env_of_t<R> &>::template callback_type<stopper>;
            ...
            void start() noexcept {
                ...
                if (not state.compare_exchange_strong(expected, state_type::emplaced, std::memory_order_acq_rel)) {
                    stop_cb.reset();
                    stdexec::set_stopped(recv);
                    return;
                }
                ...
                try {
                    snd.initiation->initiate(boost::asio::bind_cancellation_slot<boost::asio::cancellation_slot,std::function<void(Ts...)>>
                                             (cancel_sig.slot(), [&](Ts... args) { stdexec::set_value(std::move(recv), std::forward<decltype(args)>(args)...); })); // Value channel.
                }
                catch (...) {
                    stop_cb.reset();
                ...
                if (not state.compare_exchange_strong(expected, state_type::initiated, std::memory_order_acq_rel)) {
                    stop_cb.reset();
    ...
    private:
        struct asio_initiation_base { // Type erase some additional type-info of asio_initiation
            ...
            virtual void initiate(boost::asio::cancellation_slot_binder<std::function<void(Ts...)>, boost::asio::cancellation_slot>) = 0;
        };
        ...
        struct asio_initiation : asio_initiation_base {
            ...
            asio_initiation(Func init_func, std::tuple<Args...> init_args)
                : func(std::move(init_func)), args(std::move(init_args)) {}

            asio_initiation &copy() const override {
                return *new asio_initiation(func, args);
            }

            void initiate(boost::asio::cancellation_slot_binder<std::function<void(Ts...)>,boost::asio::cancellation_slot> token) override {
                std::apply(func, std::tuple_cat(std::make_tuple(std::move(token)), std::move(args)));
            }
        };

        std::unique_ptr<asio_initiation_base> initiation; // In most cases initiation is larger than 16 bits.
    };
    ...
    struct use_sender_t {
        ...
        struct executor_with_default : Executor {
            ...
            executor_with_default(const auto &ex) noexcept requires(not std::same_as<std::decay_t<decltype(ex)>, executor_with_default> and std::convertible_to<std::decay_t<decltype(ex)>, Executor>)
                : Executor(ex) {}
        };
        ...
        static auto as_default_on(auto &&obj) {
            return typename std::decay_t<decltype(obj)>::template rebind_executor<executor_with_default<typename std::decay_t<decltype(obj)>::executor_type>>::other(std::forward<decltype(obj)>(obj));
        }
    };
