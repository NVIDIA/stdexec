...
namespace nv::execution
{
  ...
  inline __device__ auto get_device_type() noexcept -> device_type
  {
...
namespace nv::execution
{
  ...
  struct stream_domain : STDEXEC::default_domain
  {
    ...
    {
      return _strm::apply_sender_for<Tag>{}(static_cast<Sender&&>(sndr),
  ...
  namespace _strm
  {
...
#else
    template <class... Ts>
    concept trivially_copyable = ((STDEXEC_IS_TRIVIALLY_COPYABLE(Ts) || std::is_reference_v<Ts>)
                                  && ...);
#endif
    ...
    class stream_pool_t
    {
     ...
     public:
      ...
      stream_pool_t(stream_pool_t const &)                    = delete;
      auto operator=(stream_pool_t const &) -> stream_pool_t& = delete;
    ...
    struct context
    {
      ...
      context(std::pmr::memory_resource* pinned_resource,
    ...
    using variant_storage_t =
      __for_each_completion_signature_t<__completion_signatures_of_t<Sender, Env>,
                                        decayed_tuple_t,
    ...
    {
      template <class Env>
        requires __callable<get_stream_provider_t, Env const &>
      auto operator()(Env const & env) const noexcept -> cudaStream_t
      {
    ...
    {
      return STDEXEC::__env::__join(STDEXEC::prop{get_stream_provider, stream_provider},
                                    static_cast<BaseEnv&&>(base_env));
    }

    template <class BaseEnv>
      requires __callable<get_stream_provider_t, BaseEnv const &>
    ...
    template <class BaseEnv>
    using stream_env_t = decltype(_strm::make_stream_env(__declval<BaseEnv>(),
        ...
        , pinned_resource_(pinned_resource)
      {
        this->execute_ = [](task_base* task) noexcept
        {
          ...
          nvexec::visit(
            ...
            {
              ::cuda::std::apply([&self]<class Tag, class... Args>(Tag, Args&... args) noexcept
        ...
        this->free_ = [](task_base* task) noexcept
        {
          ...
          self.pinned_resource_->deallocate(task,
                                            sizeof(continuation_task),
                                            std::alignment_of_v<continuation_task>);
        };
    ...
    {
      return __callable<get_stream_provider_t, Env const &>;
    }
    ...
    struct opstate_base : stream_opstate_base
    {
      ...
      void defer_temp_storage_destruction(Type* ptr)
      {
        ...
        if constexpr (!std::is_trivially_destructible_v<Type>)
        {
          ...
          stream_provider->bury(
            [ptr, stream, managed_resource]
            {
              std::int32_t device_id = cudaInvalidDeviceId;

              cudaMemRangeGetAttribute(&device_id,
                                       4,
      ...
      void propagate_completion_signal(Tag, Args&&... args) noexcept
      {
        ...
        else
        {
          continuation_kernel<OuterReceiver, Args&&...>  // by reference
            <<<1, 1, 0, get_stream()>>>(static_cast<OuterReceiver&&>(rcvr_),
                                        Tag(),
                                        static_cast<Args&&>(args)...);
        }
      }
    ...
    struct opstate : opstate_base<OuterReceiver>
    {
      ...
      opstate(CvSender&&       sender,
        ...
        , inner_op_{connect(static_cast<CvSender&&>(sender),
      ...
      opstate(CvSender&&       sender,
        ...
        , task_(host_allocate<task_t>(this->stream_provider_.status_,
                                      ...
                                      ctx.pinned_resource_)
                  .release())
        , env_(host_allocate<env_t>(this->stream_provider_.status_,
                                    ctx.pinned_resource_,
                                    this->make_env()))
        , inner_op_{connect(
      ...
      ~opstate()
      {
        ...
        {
          this->ctx_.managed_resource_->deallocate(this->temp_storage_,
                                                   InnerReceiver::memory_allocation_size());
  ...
  cudaMemPrefetchAsync(const void* dev_ptr, size_t count, int dst_device, cudaStream_t stream = 0)
  {
    return ::cudaMemPrefetchAsync(dev_ptr,
                                  count,
                                  {.type = cudaMemLocationTypeDevice, .id = dst_device},
                                  0,
                                  stream);
  }
#endif
