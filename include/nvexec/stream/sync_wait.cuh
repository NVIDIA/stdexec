        ...
        STDEXEC_TRY
        {
          ...
          if constexpr (sizeof...(Args))
          {
            if (STDEXEC_LOG_CUDA_API(cudaGetDevice(&dev_id)) == cudaSuccess)
            {
              ...
              if (STDEXEC_LOG_CUDA_API(cudaDeviceGetAttribute(&concurrent_managed_access,
    ...
    struct sync_wait_t
    {
        ...
        -> std::optional<sync_wait_result_t<Sender>>
      {
        ...
        auto        opstate = host_allocate<exit_opstate_t<Sender, receiver<Sender>>>(
          ...
          __emplace_from{[&]
                         {
                           return exit_opstate(static_cast<Sender&&>(sndr),
                                               receiver<Sender>{{}, &state, &loop},
                                               ctx);
                         }});
