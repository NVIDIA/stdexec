  ...
  namespace __counting_scopes {
    ...
    struct __scope_join_impl : __sexpr_defaults {
      ...
      static constexpr auto get_completion_signatures =
        []<class _Sender, class _Env>(const _Sender&, _Env&&) noexcept
        -> __completion_signatures_of_t<__sched_sender_of_t<_Env>, _Env> {
        return {};
      };

      template <class _Rcvr>
      struct __join_state final : __join_state_base {
        struct __rcvr_t {
        ...
        __join_state(__base_scope* __scope, _Rcvr& __rcvr)
          ...
          : __join_state_base(
              ...
              [](__join_state_base* base) noexcept {
                ...
                // if the schedule-sender is no-throw connectable with __receiver_ then
          ...
          , __op_(
              stdexec::connect(
                schedule(get_scheduler(stdexec::get_env(__rcvr))),
                __rcvr_t(__rcvr))) {
        }
      ...
      static constexpr auto get_state =
        []<class _Sndr, class _Rcvr>(_Sndr&& __sender, _Rcvr& __receiver) noexcept(
          ...
          return __join_state<_Rcvr>(scope, __receiver);
        };
    ...
    struct __base_scope {
      ...
      bool __try_associate() noexcept {
        constexpr auto makeNewBits = [](std::size_t bits) noexcept {
          ...
          assert(!__is_closed(bits) && __count(bits) < max_associations);
      ...
      void __disassociate() noexcept {
        ...
        if (__is_joined(newBits)) {
          ...
          assert(!__is_joining(newBits) || __count(newBits) > 0ul);
        }
      }
     ...
     private:
      ...
      constexpr static std::size_t
        ...
        // no high bits set
        assert(__count(__newCount << 3) == __newCount);

        // no high bits set
        assert(__state(__newState) == __newState);
