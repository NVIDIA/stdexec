...
namespace STDEXEC
{
  ...
  namespace __associate
  {
    ...
    struct __associate_data
    {
      ...
      __associate_data(__associate_data const & __other)
        noexcept(__nothrow_copy_constructible<__wrap_sender_t>
                 && noexcept(__other.__assoc_.try_associate()))
    ...
    struct associate_t
    {
      ...
      auto operator()(_Sender&& __sndr, _Token&& __token) const noexcept(
        __nothrow_constructible_from<__associate_data<std::remove_cvref_t<_Token>, _Sender>,
                                     _Token,
    ...
    struct __op_state
    {
      ...
      explicit __op_state(__associate_data_t const & __ad, _Receiver&& __rcvr)
