...
namespace STDEXEC
{
  namespace __detail
  {
    ...
    using __transform_sender_result_t =
      decltype(_DomainOrTag{}.transform_sender(_OpTag(),
                                               __declval<_Sender>(),
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////
  STDEXEC_MODULE_EXPORT_AUTHORING template <class _Tag, sender _Sender, class... _Env>
  ...
  {
    template <class _Env>
    using __starting_domain_t = __call_result_t<get_domain_t, _Env const &>;

    template <class _Tag, class _Sender, class... _Env>
    using __completing_domain_t =
      __call_result_t<get_completion_domain_t<_Tag>, env_of_t<_Sender>, _Env const &...>;
  ...
  struct get_completion_domain_t
  {
   ...
   private:
    ...
    {
      static_assert(__same_as<_Domain, __detail::__scheduler_domain_t<_Sch, _Env const &...>>,
    ...
    static constexpr auto __get_domain() noexcept
    {
      ...
      else if constexpr (__same_as<_Tag, void>)
      {
        ...
        {
          using __domain_t =
            __call_result_t<get_completion_domain_t<set_value_t>, _Attrs const &, _Env const &...>;
          return __check_domain<_Attrs, _Env...>(__domain_t{});
        }
        else
        {
   ...
   public:
    template <class _Attrs, class... _Env>
    constexpr auto
    operator()(_Attrs const &, _Env const &...) const noexcept -> __result_t<_Attrs, _Env...>
    {
      return {};
    }
  ...
  {
    template <class _Sig>
    inline static constexpr get_domain_t (*signature)(_Sig) = nullptr;
...
namespace std
{
  ...
  struct common_type<::STDEXEC::indeterminate_domain<_As...>,
