...
namespace STDEXEC {
  ...
  struct affine_on_t {
    ...
    template <class _Sender, class _Env>
    static constexpr auto transform_sender(set_value_t, _Sender&& __sndr, const _Env& __env) {
      ...
      if constexpr (!sender_in<__cv_child_t, _Env>) { // NOLINT(bugprone-branch-clone)
        ...
        return STDEXEC::__finally_(
          STDEXEC::__forward_like<_Sender>(__child), unstoppable(schedule(get_scheduler(__env))));
      }
    }
  };
