...
namespace STDEXEC {
  ...
  inline constexpr const starts_on_t& start_on = starts_on;
  ...
  using ensure_started_t
    [[deprecated("Include <exec/ensure_started.hpp> and use exec::ensure_started_t instead")]] =
      exec::ensure_started_t;

  using start_detached_t
    [[deprecated("Include <exec/start_detached.hpp> and use exec::start_detached_t instead")]] =
      exec::start_detached_t;

  using execute_t [[deprecated]] = exec::__execute_t;

  [[deprecated("Include <exec/split.hpp> and use exec::split instead")]]
  inline constexpr const exec::split_t& split = exec::split;

  [[deprecated("Include <exec/ensure_started.hpp> and use exec::ensure_started instead")]]
  inline constexpr const exec::ensure_started_t& ensure_started = exec::ensure_started;

  [[deprecated("Include <exec/start_detached.hpp> and use exec::start_detached instead")]]
  inline constexpr const exec::start_detached_t& start_detached = exec::start_detached;

  [[deprecated]]
  inline constexpr const exec::__execute_t& execute = exec::__execute;
} // namespace STDEXEC
