...
namespace STDEXEC {
   ...
   public:
    ...
    constexpr auto operator()(const _Attrs& __attrs, const _Env&... __env) const noexcept
      ...
      // scheduler for _its_ completion scheduler).
        ...
        __completes_inline<_Tag, _Attrs, _Env...> && __callable<get_scheduler_t, const _Env&...>) {
        return __check_domain<_Attrs, _Env...>(
  ...
  struct __execute_may_block_caller_t : __query<__execute_may_block_caller_t, true> {
    ...
    static constexpr void __validate() noexcept {
      static_assert(
        __std::same_as<bool, __call_result_t<__execute_may_block_caller_t, const _Attrs&>>);
      static_assert(__nothrow_callable<__execute_may_block_caller_t, const _Attrs&>);
    }
  };
  ...
  struct __mk_sch_env_t {
    ...
    constexpr auto operator()([[maybe_unused]] _Sch __sch, const _Env&... __env) const noexcept {
      if constexpr (
        ...
        return (
          __sched_env{
            get_completion_scheduler<set_value_t>(get_scheduler(__env), __hide_scheduler{__env})},
          ...);
      } else {
