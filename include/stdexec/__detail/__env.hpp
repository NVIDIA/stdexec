...
namespace stdexec {
  ...
  namespace __env {
    ...
    constexpr std::size_t __index_of(const bool* const __first, const bool* const __last) noexcept {
    ...
    struct env {
      ...
      constexpr decltype(auto) __get_1st() const noexcept {
        ...
        constexpr std::size_t __idx = __index_of(__flags, __flags + sizeof...(_Envs));
        return __tup::__get<__idx>(__tup_);
      }
