...
namespace stdexec {
  ...
  namespace __spawn_future {
    ...
    struct __future_variant<false, _Sigs...> {
      using type = __uniqued_variant_for<
        std::monostate,
    ...
    struct __spawn_future_state final
      ...
      void __complete() noexcept override {
        ...
        if (sentinel == nullptr) { // NOLINT(bugprone-branch-clone)
          ...
          // but, if so, we don't need to worry about it--invoking __callback_ like this will
      ...
      void __try_cancel() noexcept {
        ...
        //           abandons the operation (i.e. it observes the (this + 1) value and successfully
        //           replaces it with (this) before completing the receiver with set_stopped). In
        ...
        //           observes (this + 1) then we know that both __try_cancel and __consume have run
        ...
        //           performs the exchange(this, acq_rel) that is its attempt to so signal and the
    ...
    struct __spawn_future_impl : __sexpr_defaults {
      ...
      template <class _Sender>
      using __spawn_future_t = __unique_ptr_t<_Sender>::element_type;

      template <class _Sender>
      using __completions_t = __spawn_future_t<_Sender>::__completions_t;
      ...
      struct __opstate_base {
        __opstate_base(__unique_ptr_t<_Sender>&& __future) noexcept
          : __future_(std::move(__future)) {
        }

       protected:
        __unique_ptr_t<_Sender> __future_;

        struct __try_cancel {
          __spawn_future_t<_Sender>* __self_;
      ...
      struct __opstate : __opstate_base<_Sender> {
        __opstate(__unique_ptr_t<_Sender>&& __future, _Receiver& __rcvr) noexcept
          : __opstate_base<_Sender>(std::move(__future))
          , __outerRcvr_(std::addressof(__rcvr)) {
        }
        ...
        void __run() noexcept {
          constexpr bool nothrow =
            __nothrow_constructible_from<__receiver, _Receiver&, __spawn_future_t<_Sender>*>;

          _Receiver* outerRcvr = __outerRcvr_;

          try {
            std::construct_at(&__innerRcvr_, *outerRcvr, this->__future_.get());
            this->__future_.release()->__consume(__innerRcvr_);
          } catch (...) {
            // TODO: make sure the completion signatures advertise std::exception_ptr when necessary
            //
            // Consider: make __run conditionally-noexcept and handle the call to set_error in
            //           start, below; this would probably make it easier to detect whether or not
            //           we need to include std::exception_ptr in the completion signatures at the
            //           scope we need to do that in. It would probably then also make sense to
            //           get rid of the __receiver type and just construct the stop callback
            //           into the opstate itself, which would let us do away with "this 'receiver'
            //           doesn't satisfy the receiver concept".
            if constexpr (!nothrow) {
              stdexec::set_error(std::move(*outerRcvr), std::current_exception());
            }
          }
        }

       private:
        // NOTE: this type doesn't satisfy receiver
        struct __receiver {
          __receiver(_Receiver& __rcvr, __spawn_future_t<_Sender>* __future)
            noexcept(__nothrow_constructible_from<
                     __callback_t,
                     __stoken_t,
                     typename __opstate_base<_Sender>::__try_cancel
            >)
            : __rcvr_(__rcvr) {
            std::construct_at(
              &__callback_,
              stdexec::get_stop_token(stdexec::get_env(__rcvr_)),
              typename __opstate_base<_Sender>::__try_cancel{__future});
          }
         ...
         private:
          using __stoken_t = stop_token_of_t<env_of_t<_Receiver>>;
          using __callback_t =
            __stoken_t::template callback_type<typename __opstate_base<_Sender>::__try_cancel>;

          _Receiver& __rcvr_;
          union {
            __callback_t __callback_;
          };

          template <class _CPO, class... _T>
          void __set_complete(_T&&... __t) noexcept {
            std::destroy_at(&__callback_);
            _CPO{}(std::move(__rcvr_), static_cast<_T&&>(__t)...);
          }
        };

        union {
          _Receiver* __outerRcvr_;
          // it's OK not to destruct this; the _Receiver& is trivially destructible and the
          // receiver contract guarantees the stop callback will be destructed if it's ever
          // constructed
          __receiver __innerRcvr_;
        };
      };

      static constexpr auto get_state =
        []<class _Sender, class _Receiver>(_Sender&& __sndr, _Receiver& __rcvr) noexcept /* TODO */
        -> __opstate<std::remove_cvref_t<_Sender>, _Receiver> {
        auto& [_, __future] = __sndr;
        return {std::move(__future), __rcvr};
      };

      static constexpr auto start = [](auto& __state, auto&) noexcept {
        __state.__run();
      };
    };
  } // namespace __spawn_future
