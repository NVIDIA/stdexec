...
namespace STDEXEC {
  ...
  namespace __spawn_future {
    ...
    inline constexpr bool __stop_callback_is_nothrow_constructible = __nothrow_constructible_from<
      typename stop_token_of_t<_Env>::template callback_type<__future_stop_callback>,
    ...
    struct __future_operation : __future_operation_base<_Sender> {
      ...
      void __run() noexcept(__stop_callback_is_nothrow_constructible<env_of_t<_Receiver>>) {
        ...
        std::construct_at(
          &__callback_,
          STDEXEC::get_stop_token(STDEXEC::get_env(__outerRcvr_)),
     ...
     private:
      ...
      void __complete(_Ts&&... __ts) noexcept {
        ...
        _CPO{}(std::move(__outerRcvr_), static_cast<_Ts&&>(__ts)...);
      }
    };
    ...
    struct __spawn_future_impl : __sexpr_defaults {
      ...
      static constexpr auto start = [](auto& __state, auto& __receiver) noexcept {
        ...
        try {
          ...
          if constexpr (!nothrow) {
            STDEXEC::set_error(std::move(__receiver), std::current_exception());
          }
        }
      };
    };
