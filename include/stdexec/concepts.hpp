...
namespace stdexec::__std_concepts {
  ...
  template <class _A, class _B>
  concept derived_from = std::is_base_of_v<_B, _A>
                      && std::is_convertible_v<const volatile _A *, const volatile _B *>;

  template <class _From, class _To>
  concept convertible_to = std::is_convertible_v<_From, _To> && requires(_From (&__fun)()) {
...
namespace stdexec {
  ...
  // lot of templates.
  template <class _Ty>
  inline constexpr bool __destructible_ = requires {
  ...
  template <class _T>
  concept copy_constructible = move_constructible<_T> && constructible_from<_T, _T const &>;

  template <class _T>
  concept __movable_value = move_constructible<decay_t<_T>> && constructible_from<decay_t<_T>, _T>;
  ...
  template <class _Ty, template <class...> class _T>
  concept __is_not_instance_of = !__is_instance_of<_Ty, _T>;
