...
namespace stdexec {
  ...
  namespace __env {
    ...
    struct __join_env_t {
      ...
      decltype(auto)
        operator()(_Env0 && __env0, _Env1 && __env1, _Env2 && __env2, _Envs&&... __envs)
          const noexcept {
        ...
        try {
          if constexpr (same_as<__result_t, void>)
            co_await (
              ...
              __co_call(stdexec::set_value, (_Receiver&&) __rcvr));
          else
            co_await __co_call(
              stdexec::set_value, (_Receiver&&) __rcvr, co_await (_Awaitable&&) __awaitable);
        } catch (...) {
  ...
  STDEXEC_DEFINE_CPO(struct connect_t, connect) {
    ...
    auto operator()(_Sender&& __sndr, _Receiver&& __rcvr) const
      ...
      if constexpr (__connectable_with_tag_invoke<_Sender, _Receiver>) {
        ...
        return tag_invoke(
          connect_t{}, transform_sender(__domain, (_Sender&&) __sndr, __env), (_Receiver&&) __rcvr);
      } else if constexpr (__connectable_with_co_await<_Sender, _Receiver>) {
    ...
    struct receiver_adaptor {
      class __t
        ...
        STDEXEC_DETAIL_CUDACC_HOST_DEVICE //
          ...
          static_assert(
            noexcept(STDEXEC_CALL_MEMBER(set_value, (_Derived&&) __self, (_As&&) __as...)));
        ...
        (_Dp, set_value)
          && tag_invocable<_SetValue, __base_t<_Dp>, _As...> STDEXEC_DETAIL_CUDACC_HOST_DEVICE //
        ...
        STDEXEC_DETAIL_CUDACC_HOST_DEVICE //
          ...
          static_assert(
            noexcept(STDEXEC_CALL_MEMBER(set_error, (_Derived&&) __self, (_Error&&) __err)));
        ...
        template <same_as<set_error_t> _SetError, class _Error, class _Dp = _Derived>
          requires STDEXEC_MISSING_MEMBER
        (_Dp, set_error)
          && tag_invocable<_SetError, __base_t<_Dp>, _Error> STDEXEC_DETAIL_CUDACC_HOST_DEVICE //
        ...
        template <same_as<set_stopped_t> _SetStopped, class _Dp = _Derived>
          requires STDEXEC_MISSING_MEMBER
        (_Dp, set_stopped)
          && tag_invocable<_SetStopped, __base_t<_Dp>> STDEXEC_DETAIL_CUDACC_HOST_DEVICE //
     ...
     public:
      struct __scheduler {
       ...
       private:
        ...
        STDEXEC_DEFINE_CUSTOM(__schedule_task schedule)(
          this const __scheduler& __self,
          schedule_t) noexcept {
  ...
  namespace __sync_wait {
    ...
    struct __env : __result_of<__make_env, run_loop&> {
      __env(run_loop& __loop) noexcept
        : __result_of<__make_env, run_loop&> {
        __sync_wait::__make_env(__loop)
      }
