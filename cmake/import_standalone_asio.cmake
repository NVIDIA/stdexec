...
#
# Importing standalone asio can't be done via rapids-cpm, because the library
# has no cmake build setup. But still it can be imported with CPM.
#
...
function(import_standalone_asio)
  ...
  set(multi_args "")
  cmake_parse_arguments(IMPORT_STANDALONE_ASIO "${options}" "${args}"
                        "${multi_args}" ${ARGN})
  ...
  cpmfindpackage(
    ...
    asio
    VERSION
    ${IMPORT_STANDALONE_ASIO_VERSION}
    GIT_TAG
    ${IMPORT_STANDALONE_ASIO_TAG}
    GITHUB_REPOSITORY
    chriskohlhoff/asio)

  # ASIO doesn't use CMake, we have to configure it manually. Extra notes for
  # using on Windows:
  #
  # 1) If _WIN32_WINNT is not set, ASIO assumes _WIN32_WINNT=0x0501, i.e.
  # Windows XP target, which is definitely not the platform which most users
  # target.
  #
