...
int main() {
  ...
  using winner = __compl_sigs::__maybe_for_all_sigs<
    __completion_signatures_of_t<
      decltype(snd), empty_env
    >,
    __select_completions_for_or<set_value_t, __qf<set_value_t>>,
    __remove< __, __q<completion_signatures> >
  >;
  using testtwo = __select_completions_for_or<set_value_t, __qf<set_value_t>>;
  using testthree = __compl_sigs::__for_all_sigs<
  
  >;
  stdexec::print(std::declval<winner>());
  using stdexec::__tag_invoke::tag_invoke;
...
using complet = __gather_completions_for<
  ...
  type_print<complet> heybro;
    ...
    __gather_signal,
    set_value_t,
    __completion_signatures_of_t<decltype(snd), empty_env>,
    nvexec::_strm::reduce_t::_set_value_t<decltype(span), float>,
    __q<__compl_sigs::__ensure_concat>>;
  type_print<eval> heybro;

  using second = __only_gather_signal<set_value_t, __completion_signatures_of_t<decltype(snd), empty_env>>;
    type_print<second> heybro;
