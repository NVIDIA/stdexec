...
template <class _Tag, _Captures&&... __captures>
auto useless_fn() = {
  [... __captures = (_Captures&&)
     __captures]<class _Cvref, class _Fun>(_Cvref, _Fun&& __fun) mutable    //
  noexcept(__nothrow_callable<_Fun, _Tag, __minvoke<_Cvref, _Captures>...>) //
  -> decltype(auto)                                                         //
  requires __callable<_Fun, _Tag, __minvoke<_Cvref, _Captures>...>{
    return ((_Fun&&) __fun)(_Tag(), const_cast<__minvoke<_Cvref, _Captures>&&>(__captures)...);
}
;
}
...
int main() {
  ...
  __mtry_eval<
    ...
    __basic_sender<decltype(useless_fn)>,
    test_env,
    set_value_t,
