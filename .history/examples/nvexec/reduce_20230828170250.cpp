...
int main() {
  ...
  // stdexec::print(snd);
  using my_try_v_t = __try_value_types_of_t<
    decltype(snd),
    test_env,
    nvexec::_strm::reduce_t::_set_value_t<decltype(span), float> >;

  using eval =  __mtry_eval<__try_value_types_of_t, decltype(snd), test_env,  nvexec::_strm::reduce_t::_set_value_t<decltype(span), float>, __q<__ensure_concat>>;
  using stdexec::__tag_invoke::tag_invoke;
