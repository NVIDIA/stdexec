...
int main() {
  ...
  using MyCompletionSignature =
    __try_make_completion_signatures< MySender, MyEnv, MySigs, MySetValue, MySetError, MySetStopped >;
  ...
  //static_assert(sender_in<decltype(snd), test_env>, "Failed");
  stdexec::print(snd);
  using my_try_v_t =
    __try_value_types_of_t<decltype(snd), test_env, __q<nvexec::_strm::reduce_t::_set_value_t<decltype(first), float> >>;
