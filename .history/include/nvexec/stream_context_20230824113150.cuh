  ...
  namespace STDEXEC_STREAM_DETAIL_NS {
    ...
    struct stream_scheduler {
      ...
      friend stream_domain tag_invoke(get_domain_t, const stream_scheduler& sch) noexcept {
        ...
        friend auto tag_invoke(stdexec::get_completion_signatures_t, Self&& self, Env&& env)
          -> completion_signatures<Self, Env>
          {
            completion_signatures<Self, Env> now{};
            (print)(now);
            nvexec::print(now);
   return { std::forward<Self>(self), std::forward<Env>(env) };
          }
