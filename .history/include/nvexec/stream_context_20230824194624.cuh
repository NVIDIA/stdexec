    ...
    struct stream_scheduler {
      ...
      struct sender_ {
        ...
        struct __t : stream_sender_base {
          ...
          friend auto tag_invoke(stdexec::get_completion_signatures_t, Self&& self, Env&& env) -> completion_signatures<Self, Env> {
                  return {};            
                }
          template <class R>
      ...
      template <class Self, class Env>
      auto tag_invoke(stdexec::get_completion_signatures_t, Self&& self, Env&& env) -> completion_signatures<Self, Env> {
        return {};            
      }

      // template <class S, class InitT, class Fn>
      // reduce_sender_t<S, InitT, Fn> make_reduce_sender_(S&& sndr, InitT initT, Fn     fun)    const {
      //   return reduce_sender_t<S, InitT, Fn>{(S&&) sndr, initT, (Fn&&) fun};
      // } // doesn't take a pool like this. 


      // template <stdexec::sender S, std::integral InitT, class Fn>
      // reduce_sender_t<S, InitT, Fn> tag_invoke(STDEXEC_STREAM_DETAIL_NS::reduce_t, scheduler sch, S&&  sndr, InitT initT, Fn fun) noexcept {
      //   return sch.make_reduce_sender_((S&&) sndr, initT, (Fn&&) fun);
      // }
