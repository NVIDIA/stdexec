...
namespace nvexec {
  namespace STDEXEC_STREAM_DETAIL_NS {
    ...
    struct stream_domain {
      // l
      template < class Sender, typename... Senders, typename InitT, typename Fun>
      auto transform_sender_recursive(Sender&& head, Senders&&... tail, InitT&& initT, Fun&& fun) const {
        static_assert(
            constructible_from<__decay_t<reduce_sender_t<Sender, InitT, Fun>>, Sender, InitT, Fun>,
            "sender cannot be constructed!"
        );
        
        if constexpr (sizeof...(tail) == 0) {
          reduce_sender_t<Sender, InitT, Fun> cur{ (Sender&&) head, initT,  std::move(fun) };
          return cur;
        } else {
        ...
        return stdexec::__sender_apply(
          ...
          [&]<class Tag, class Data, class First, class... Rest>(
            Tag, Data&& data, First first, Rest... rest) {
              stdexec::print(std::declval<First>());
            auto initT = stdexec::__nth_member<0>()((Data&&) data);
            auto fun = stdexec::__nth_member<1>()((Data&&) data);
            reduce_sender_t<First, decltype(initT), decltype(fun)> test{};
            return transform_sender_recursive(first,
