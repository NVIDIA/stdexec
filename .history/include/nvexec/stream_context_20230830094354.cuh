...
namespace nvexec {
  namespace STDEXEC_STREAM_DETAIL_NS {
    ...
    struct stream_domain {
      ...
      template <
        typename FirstInnerSender,
        typename... RestInnerSenders,
        typename InitT,
        typename Fun,
        typename Sched>
      auto transform_sender_recursive(
        FirstInnerSender&& firstInnerSender,
        RestInnerSenders&&... restInnerSenders,
        InitT&& initT,
        Fun&& fun) {
          // quite a few problems i think i see. first and fore most i'm not sure if this transforms every sender with the exception of the last sender. 
        if constexpr (sizeof...(RestInnerSenders) == 0) {
          return reduce_sender_t<FirstInnerSender, InitT, Fun>{
            std::forward<FirstInnerSender>(firstInnerSender),
            std::forward<InitT>(initT),
            std::forward<Fun>(fun)};
        } else {
          return transform_sender_recursive(
            reduce_sender_t<>{},
            std::forward<RestInnerSenders>(restInnerSenders)...,
            std::forward<InitT>(initT),
            std::forward<Fun>(fun));
        }
      }
