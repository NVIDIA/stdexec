    ...
    struct stream_domain {
      ...
      auto reconstitute() {
        return [](auto data, auto sender) {
          ...
          return reduce_sender_t<decltype(sender), decltype(initT), decltype(fun)>({}, sender, initT, fun);
        };
      }

      // quite a few things that i'm not sure of.
      // i think it would make the most sense for the data to be passed in to reconstitute. but what doesn't
      ...
      auto transform_sender(Sender&& sndr, const Env& env = {}) const noexcept {
        ...
        return stdexec::__sender_apply(
          ...
          []<class Tag, class Data, class... Children>(Tag, Data&& data, Children... children) {
            auto initT = stdexec::__nth_member<0>()((Data&&) data);
            auto fun = stdexec::__nth_member<1>()((Data&&) data);
            return reconstitute<reduce_sender_t>()(data, transform_sender(children, env)...);
          });
      }

      d template <class _Sender, class _Env = empty_env>
