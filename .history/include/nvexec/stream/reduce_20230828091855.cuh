  ...
  namespace STDEXEC_STREAM_DETAIL_NS {
    namespace reduce_ {
      ...
      struct sender_t
        ...
        template <class _Env> //__decays_to<__basic_sender>
        friend auto tag_invoke(get_completion_signatures_t, SenderId&&, _Env&&)
          -> simple<SenderId, _Env, InitT, Fun>
          requires true {}
 

        template <class _Child, class _Env, class _InitT, class _Fun> //__decays_to<__basic_sender>
        friend auto tag_invoke(get_completion_signatures_t, _Child&&, _Env&&)
          -> simple<_Child, _Env, _InitT, _Fun>
          requires true;

        template <class _Child, class _Env, class _InitT, class _Fun> //__decays_to<__basic_sender>
        friend auto tag_invoke(get_completion_signatures_t, _Child&&, _Env&&, _InitT)
          -> simple<_Child, _Env, _InitT, _Fun>
          requires true;

        template <class _Child, class _Env, class _InitT, class _Fun> //__decays_to<__basic_sender>
        friend auto tag_invoke(get_completion_signatures_t, _Child&&, _Env&&, _InitT, _Fun)
          -> simple<_Child, _Env, _InitT, _Fun>
          requires true;

        template <class _Child, class _Env, class _InitT, class _Fun> //__decays_to<__basic_sender>
        friend auto tag_invoke(get_completion_signatures_t) -> simple<_Child, _Env, _InitT, _Fun>
          requires true;


        template <class _Child, class _Env, class _InitT, class _Fun> //__decays_to<__basic_sender>
        friend auto tag_invoke(get_completion_signatures_t, _Child&&)
          -> simple<_Child, _Env, _InitT, _Fun>
          requires true;
      };
    }
    ...
    struct reduce_t {
      ...
      template <class CvRefSender, class _Env, class InitT, class _Fun>
      using pls = //
        __try_make_completion_signatures<
          CvRefSender, // ?
          _Env,
      ...
      static auto get_completion_signatures(_Sender&& __sndr, _Env&& env) {
        return __sender_apply(
          ...
          []<class _Data, class _Child>(reduce_t tag, _Data data, _Child&& child) {
            ...
            static_assert(__valid< simple, _Child, _Env, _InitT, _Fun>, "hey");

            return simple<_Child, _Env, _InitT, _Fun>();
          });
      }
