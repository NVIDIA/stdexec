  ...
  namespace STDEXEC_STREAM_DETAIL_NS {
    namespace reduce_ {
      ...
      struct sender_t
        ...
        template <class Child, class Self, class Env>
        friend auto tag_invoke(get_completion_signatures_t, Self&&, Env&&)
          -> __completion_signatures<Child, Self, Env>
          requires true
        {
          return {};
        }
      ...
      static auto get_completion_signatures(_Sender&& __sndr, _Env&& env) {
        return __sender_apply(
          ...
          []<class _Data, class _Child>(reduce_t tag, _Data data, _Child&& child) {
            using _Shape = decltype(_Data::__initT_);
            ...
            if constexpr (__valid< __completion_signatures, _Child, _Env, _Fun>) {
              stdexec::print(__sndr);
              stdexec::print(tag);
              stdexec::print(data);
              stdexec::print(child);
              using __completion_signatures_ttt = reduce_::sender_t<_Sender, _InitT, _Fun>::
                template _completion_signatures_t<_Child, _Env>;
              return __completion_signatures_ttt();
              return __completion_signatures< _Child, _Env>();
