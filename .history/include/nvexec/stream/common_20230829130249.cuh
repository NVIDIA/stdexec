...
[[gnu::deprecated]]
  void prints(auto&&...) {}
    ...
    stream_op_state_t<Sender, inner_receiver_t<ReceiverProvider, OuterReceiver>, OuterReceiver>
      ...
      // uses get_completion_scheduler parameterized over stdexec::set_value_t to query
      // for the scheduler. I think this means tag_invoke(get_completion_scheduler, get_env, sender) is called but i'm not 100% sure.
      // then returns a stream_op_state_t with sndr, out_receiver, receiver_provider, and context_state as parameters. This seems to rely on another constructor. 
