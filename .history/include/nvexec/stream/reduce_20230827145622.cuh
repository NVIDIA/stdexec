...
namespace nvexec {
  namespace STDEXEC_STREAM_DETAIL_NS {
    ...
    struct reduce_t {
      ...
      template <class Child, class Sender, class Self, class Env>
      using pls_work_compl_sigs = //
        __try_make_completion_signatures<
          __copy_cvref_t<Child, Sender>, // ?
          Env,
          stdexec::completion_signatures<
            set_value_t(), //
            set_stopped_t() >,
        ...
        return __sender_apply(
          ...
          []<class _Data, class _Child>(reduce_t tag, _Data data, _Child&& child) {
            ...
            static_assert(
              ...
              "failed");
            if constexpr (stdexec::__valid< pls_work_compl_sigs, _Child, _Env, _Fun>) {
              return pls_work_compl_sigs<_Child, _Sender, _InitT, _Fun>();
      ...
      template <class Child, class Sender, class Env> //__decays_to<__basic_sender>
      friend auto tag_invoke(get_completion_signatures_t, Sender&&, Env&&)
        -> pls_work_compl_sigs<Child, Env, cub::Sum>
        requires true;
