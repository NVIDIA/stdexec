...
namespace nvexec {
  namespace STDEXEC_STREAM_DETAIL_NS {
    ...
    struct reduce_t {
        ...
        return __sender_apply(
          ...
          []<class _Data, class _Child>(reduce_t tag, _Data data, _Child&& child) {
            ...
            if constexpr (stdexec::__valid< pls_work_compl_sigs, _Child, _Env, _Fun>) {
              return pls_work_compl_sigs<_Sender, _Fun>();
      ...
      template <class _Child, class Env> //__decays_to<__basic_sender>
      friend auto tag_invoke(get_completion_signatures_t, _Child&&, Env&&)
        -> pls_work_compl_sigs<_Child, Env>
        requires true;
