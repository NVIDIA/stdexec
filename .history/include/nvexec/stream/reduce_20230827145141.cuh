  ...
  namespace STDEXEC_STREAM_DETAIL_NS {
    ...
    struct reduce_t {
      ...
      static auto get_completion_signatures(_Sender&& __sndr, _Env&& env) {
        return __sender_apply(
          ...
          []<class _Data, class _Child>(reduce_t tag, _Data data, _Child&& child) {
            ...
            template <class _ChildInner, class _InitTInner, class _FunInner>
            using _set_value_local_template = stdexec::completion_signatures<set_value_t(
              ::std::add_lvalue_reference_t<
                typename nvexec::_strm::__algo_range_init_fun::
                  binary_invoke_result_t<_ChildInner, _InitTInner, _FunInner>>)>;

            template <class _Ty>
            using _set_error_local_template =
              stdexec::completion_signatures<stdexec::set_error_t(__decay_t<cudaError>&&)>;

            using pls_work_compl_sigs_local = //
              __try_make_completion_signatures<
                __copy_cvref_t<_Child, _Sender>, // ?
                _Env,
                stdexec::completion_signatures<
                  set_value_t(), //
                  set_stopped_t() >,
                __q<_set_value_local_template>,
                __q<_set_error_local_template> >;

            static_assert(
              stdexec::__valid<
                stdexec::__compl_sigs::__compl_sigs_impl,
                _Child,
                _Env,
            ...
            if constexpr (stdexec::__valid<pls_work_compl_sigs_local, _Child, _Env, _Fun>) {
              return pls_work_compl_sigs_local();
      ...
      friend auto tag_invoke(get_completion_signatures_t, Sender&&, Env&&)
        -> pls_work_compl_sigs<Sender, Env, cub::Sum>
        requires true;
