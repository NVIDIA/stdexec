  ...
  namespace STDEXEC_STREAM_DETAIL_NS {
    namespace reduce_ {
      ...
      struct sender_t
        ...
        template <class _Ty>
        using _set_error_t = completion_signatures<set_error_t(__decay_t<cudaError>&&)>;
        template <class Range>
    ...
    struct reduce_t {
      ...
      using _set_value_t = completion_signatures<set_value_t(
        ::std::add_lvalue_reference_t< typename nvexec::_strm::__algo_range_init_fun::
                                         binary_invoke_result_t<Range, InitT, Fun>>)>;
      ...
      template <class _CvrefSender, class _Env, class _Fun>
      using __completion_signatures = //
        __try_make_completion_signatures< _CvrefSender, _Env, completion_signatures<set_stopped_t()>>;

      template <class Self, class Env>
      friend auto tag_invoke(get_completion_signatures_t, Self&& snd, Env&&)
        -> __completion_signatures<Self, Env, cub::Sum> {
        stdexec::print(snd);
        stdexec::type_printer<Self>();
        return {};
      }
      ...
      static auto get_completion_signatures(_Sender&& __sndr, _Env&& env) {
        return __sender_apply(
          ...
          []<class _Data, class _Child>(reduce_t tag, _Data data, _Child&& child) {
            ...
            static_assert(
              __valid< __completion_signatures, _Child, _Env, _Fun>, "failed assertion");
            if constexpr (__valid< __completion_signatures, _Child, _Env, _Fun>) {

              using __completion_signatures_ttt = reduce_::sender_t<_Child, _InitT, _Fun>::
                template _completion_signatures_t<_Child, _Env>;
              return __completion_signatures_ttt();
      ...
      template <class Sender, class Self, class Env>
      using yo_compl = //
        __try_make_completion_signatures<
      ...
      template <class Sender, class Env> //__decays_to<__basic_sender>
      friend auto tag_invoke(get_completion_signatures_t, Sender&&, Env&&)
        -> yo_compl<Sender, Env, cub::Sum>
        requires true
      {
        return {};
      }
...
/*
      ...
      //       using _InitT = decltype(_Data::__initT_);
      //       using _Fun = decltype(_Data::__fun_);
      //       using __completion_signatures = reduce_::sender_t<_Sender, _InitT, _Fun>::
      //         template _completion_signatures_t<_Child, _Env>;
      ...
      static auto get_completion_signatures(_Sender&& __sndr, _Env&&) {
        return stdexec::__sender_apply(
          (_Sender&&) __sndr, [&]<class _Data, class _Child>(reduce_t, _Data, _Child&&) {
            using _InitT = decltype(_Data::__initT_);
            using _Fun = decltype(_Data::__fun_);
            using __completion_signatures = reduce_::sender_t<_Child, _InitT, _Fun>::
              template _completion_signatures_t<_Child, _Env>;
            return __completion_signatures();
          });
      }
