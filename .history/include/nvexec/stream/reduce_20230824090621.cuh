...
namespace nvexec {
  namespace STDEXEC_STREAM_DETAIL_NS {
    namespace reduce_ {
      ...
      struct receiver_t   : public __algo_range_init_fun::receiver_t<
            SenderId,
            ReceiverId,
            InitT,
            Fun,
      ...
      struct __operation {
        ...
        struct __t : __immovable {
          ...
          friend __operation tag_invoke(connect_t, __t& __self,
                                         _Receivr __rcvr) noexcept {
            return {((_Self &&) __self).sender, (_Receivr &&) __rcvr};
          }

          template <__decays_to<__t> _Self, class _Receivr>
          friend __operation tag_invoke(connect_t, _Self && __self,
                                        _Receivr __rcvr) noexcept {
            return {((_Self &&) __self).sender, (_Receivr &&) __rcvr};
          }
        };
      ...
      struct sender_t
        ...
        using receiver_t =
          stdexec::__t<reduce_::receiver_t< SenderId,
        ...
        friend __operation<_Self, _Receivr, InitT, Fun> tag_invoke(connect_t, _Self && __self,
                                      _Receivr __rcvr) noexcept {
          return {((_Self &&) __self).sender, (_Receivr &&) __rcvr};
        }

        template <stdexec::__decays_to<sender_t> _Self, class _Env>
        friend auto tag_invoke(stdexec::get_completion_signatures_t, _Self&&, _Env&&)
          -> stdexec::completion_signatures<stdexec::set_value_t(InitT)> { 
             return {};
          }
      };
    }
      ...
      static auto get_completion_signatures(_Sender&& __sndr, _Env&&) {
        return stdexec::__sender_apply(
          (_Sender&&) __sndr, [&]<class _Data, class _Child>(reduce_t, _Data, _Child&&) {
            ...
            using __completion_signatures = reduce_::sender_t<_Sender, _InitT, _Fun>::__t::template completion_signatures<_Sender, _Env>;
            if constexpr (__valid<__completion_signaturesss, _Child, _Env, _InitT, _Fun>) {
              return __completion_signatures< _Child, _Env, _InitT, _Fun>();
