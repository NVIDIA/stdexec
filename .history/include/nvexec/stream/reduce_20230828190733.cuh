  ...
  namespace STDEXEC_STREAM_DETAIL_NS {
    namespace reduce_ {
      ...
      struct sender_t
        ...
        using _set_value_t = stdexec::completion_signatures<set_value_t(
          ::std::add_lvalue_reference_t< typename __algo_range_init_fun::_force_minvoke<
            __q<binary_invoke_result_t<Range, InitT, Fun>>>>)>;
    ...
    struct reduce_t {
      ...
      auto operator()(Sender&& sndr, InitT init, Fun fun) const {
        stdexec::print(init);
        type_printer<InitT> whatTypeIsInit;
      ...
      static auto get_completion_signatures(_Sender&& __sndr, _Env&& env) {
        return __sender_apply(
          ...
          []<class _Data, class _Child>(reduce_t tag, _Data data, _Child&& child) {
            ...
            static_assert(stdexec::__mvalid< pls_work, _Child, _Env, _InitT, _Fun>, "hey");

            type_printer<__detail::__name_of<_Child>> aldkfjasd;
            return pls_work<_Child, _Env, _InitT, _Fun>();
          });
      }
      ...
      friend auto tag_invoke(get_completion_signatures_t, Sender&&, Env&&) {
        pls<Sender, Env, InitT, Fun> our_ret;
        type_print<decltype(our_ret)> hi;
        return our_ret;
      }

      template <stdexec::__decays_to<reduce_t> Sender, class Env, class InitT, class Fun>
      friend auto tag_invoke(get_completion_signatures_t, Sender&&, Env&&, InitT, Fun) {
        return pls<Sender, Env, InitT, Fun>{};
      }
