...
auto transform_sender(Sender&& sndr, const Env& env = {}) const noexcept {
    return stdexec::__sender_apply(
        (Sender&&) sndr,
        [&]<class Tag, class Data, class First, class ... Rest>(Tag, Data&& data, First firstChild, Rest ... inner) {
            ...
            if constexpr (sizeof...(Rest) == 0) {
                return reduce_sender_t<Sender, decltype(initT), decltype(fun)>{
                    (Sender&&) sndr, initT, std::move(fun)};
            } else {
                return transform_sender(
                    transform_sender(firstChild, env), // process the first child
                    transform_sender(inner..., env)    // recursively process the rest
                );
            }
        });
}
