        ...
        return stdexec::__sender_apply(
          ...
          [&]<class Tag, class Data, class... Children>(Tag, Data&& data, Children&& children) {
            ...
            if constexpr (sizeof...(children) == 0) {
              return reduce_sender_t<Sender, decltype(initT), decltype(fun)>{
                {}, (Sender&&) sndr, std::move(initT), std::move(fun)},
              env);
            } 
          //  stdexec::__transform<transform_sender, children ...> probably not right. Or maybe... it is. No.
          // There are a few problems to account here. It would make sense on a single type. 
          // We want to do a __transform<transform_sender, sender> 
          // for every sender. 
          // The problem is I don't know how to access a type on a single basis.
          // __transform will only accept two parameters
            transform_sender(
              reduce_sender_t<Children, decltype(initT), decltype(fun)>{
                {}, (Children&&) children, std::move(initT), std::move(fun)},
              env);
          });
      }
