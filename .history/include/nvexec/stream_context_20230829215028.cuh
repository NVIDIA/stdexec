...
namespace nvexec {
  namespace STDEXEC_STREAM_DETAIL_NS {
    ...
    struct stream_domain {
      ...
      // the problem still boils down to ; How do I pass it 1 by 1
      template <stdexec::__lazy_sender_for<reduce_t> Sender, class ... Children, class Env = empty_env>
      auto transform_sender(Sender&& head, Children&& children ... children, const Env& env = {}) const noexcept {
      ...
      // the problem still boils down to ; How do I pass it 1 by 1
      template <class Sender, class ... Children, class Env = empty_env>
      auto transform_sender(Sender&& head, Children&& children ... children, const Env& env = {}) const noexcept {
        if constexpr (sizeof...(children)==0) {
          return reduce_sender_t<Sender, decltype(initT), decltype(fun)>{
                (Sender&&) head, initT, std::move(fun)};
