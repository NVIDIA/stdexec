...
namespace nvexec {
  namespace STDEXEC_STREAM_DETAIL_NS {
    ...
    struct stream_domain {
      ...
      template <typename FirstInnerSender, typename... RestInnerSenders, typename InitT, typename Fun, typename Sched>
auto transform_sender_recursive(FirstInnerSender&& firstInnerSender, RestInnerSenders&&... restInnerSenders,
                                InitT&& initT, Fun&& fun, Sched&& sched) {
    if constexpr (sizeof...(RestInnerSenders) == 0) {
        return reduce_sender_t<FirstInnerSender, InitT, Fun>{
            std::forward<FirstInnerSender>(firstInnerSender), std::forward<InitT>(initT), std::forward<Fun>(fun)};
    } else {
        return transform_sender_recursive(std::forward<RestInnerSenders>(restInnerSenders)..., 
                                          std::forward<InitT>(initT), std::forward<Fun>(fun), std::forward<Sched>(sched));
    }
}
      ...
      auto transform_sender(Sender&& sndr, const Env& env = {}) const noexcept {
        return stdexec::__sender_apply(
          ...
          [&]<class Tag, class Data, class First, class... Rest>(
            Tag, Data&& data, First firstChild, Rest... inner) {
            auto initT = stdexec::__nth_member<0>()((Data&&) data);
            auto fun = stdexec::__nth_member<1>()((Data&&) data);

            if constexpr (sizeof...(Rest) == 0) {
              return reduce_sender_t<First, decltype(initT), decltype(fun)>{
                (First&&) firstChild, initT, std::move(fun)};
            } else {
              return transform_sender(inner..., env);
            }
          });
      }
    };
