  ...
  namespace STDEXEC_STREAM_DETAIL_NS {
    ...
    struct stream_domain {
      ...
      template <
        ...
        typename Fun,
        typename Sched>
      auto transform_sender_recursive(Sender&& head, Senders&&... tail, InitT&& initT, Fun&& fun) {
        ...
        if constexpr (sizeof...(tail) == 0) {
          return reduce_sender_t<Sender, InitT, Fun>{
            std::forward<Sender>(head), std::forward<InitT>(initT), std::forward<Fun>(fun)};
        } else {
      ...
      auto transform_sender(Sender&& sndr, const Env& env = {}) const noexcept {
        return stdexec::__sender_apply(
          ...
          [&]<class Tag, class Data, class First, class... Rest>(
            ...
            return transform_sender_recursive(firstChild, inner..., initT, std::move(fun));
          });
      }
    };
