...
namespace nvexec {
      ...
      auto transform_sender(Sender&& sndr, const Env& env = {}) const noexcept {
        ...
        if constexpr (sizeof...(children) == 0) {
                return reduce_sender_t<Sender, decltype(initT), decltype(fun)>
                  {
                      {}, (Sender&&) sndr, std::move(initT), std::move(fun)}, env)
        };
            } else {
        auto recurse_children = [&]<class First, class... Rest>(First&& first, Rest&&... rest) {
          transform_sender(
            reduce_sender_t<First, decltype(initT), decltype(fun)>{
              {}, std::forward<First>(first), std::move(initT), std::move(fun)},
            env);

          if constexpr (sizeof...(Rest) > 0) {
            recurse_children(std::forward<Rest>(rest)...);
          }
        };

        recurse_children((Children&&) children...);
            }
          });
    }
  ...
  struct stream_scheduler {
    ...
    friend auto tag_invoke(transfer_when_all_t, const stream_scheduler& sch, Senders&&... sndrs) //
      noexcept {
    ...
    template <sender S>
    friend split_sender_th<S> tag_invoke(split_t, const stream_scheduler& sch, S&& sndr) noexcept {
