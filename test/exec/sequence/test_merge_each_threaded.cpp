...
namespace {
  ...
  TEST_CASE(
    ...
    auto v = ex::sync_wait(ignore_all_values(merged | continues_each_on(sched0) | then_each([&count](int x){
      ++count;
  ...
  TEST_CASE(
    ...
    auto error_after_on = [sched0, elapsed_ms](auto sched, duration_of_t<decltype(sched)> after, auto error) {
        return schedule_after(sched, after)
          | stdexec::continues_on(sched0) // serializes output on the sched0 strand
    ...
    auto sequences = merge(
      ...
      ex::just(error_after_on(sched1, 40ms,
        std::runtime_error{"failed sequence "})) // no items
    );
    ...
    auto v = ex::sync_wait(
      ex::when_all(
        ...
        ignore_all_values(merged
          | continues_each_on(sched0) // serializes output on the sched0 strand
          | then_each([&count, elapsed_ms](int x){
              ++count;
              UNSCOPED_INFO("item: " << x
                << ", arrived at: " << std::setw(3) << elapsed_ms()
                << "ms, on thread id: " << std::this_thread::get_id());
              return count;
            }))
        ));
  ...
  TEST_CASE(
    ...
    auto sequences = merge(
      ...
      ex::just(empty_sequence()),                // no items
      ex::just(range(200, 220)),                 // int items
      ex::just(error_after_on(sched1, 50ms,
        std::runtime_error{"failed sequence "})) // no items
    );
