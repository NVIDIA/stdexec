...
namespace
{
  ...
  struct null_receiver
  {
    ...
    {
      return STDEXEC::upon_error(STDEXEC::then(static_cast<_Item&&>(__item), ignore_values_fn_t{}),
                                 ignore_values_fn_t{});
    }
  };
  ...
  static constexpr auto flat_map = [](auto&& f)
  {
    ...
    {
      return merge_each(exec::transform_each(static_cast<decltype(sequence)&&>(sequence),
  ...
  {
    return exec::sequence(ex::read_env(ex::get_stop_token)
                            | STDEXEC::then(
                              [message](auto&& token) noexcept
                              {
                                UNSCOPED_INFO(message
                                              << (token.stop_requested() ? ", stop was requested"
                                                                         : ", stop not requested")
  ...
  auto log_sequence = [](auto sequence, auto message)
  {
    return sequence
         | then_each(
             [message](auto&& value) mutable noexcept
             {
  ...
  TEST_CASE("merge_each - merge_each sender merges all items from multiple threads",
            "[sequence_senders][single_thread_context][merge_each][merge][iterate]")
  {
    ...
    auto sequences = merge(ex::just(range(100, 120) | continues_each_on(sched1)),
    ...
    auto v = ex::sync_wait(ignore_all_values(
      merged | continues_each_on(sched0)
      | then_each(
        [&count](int x)
        {
          ++count;
  ...
  TEST_CASE("merge_each - merge_each sender stops on failed item while merging all items from "
            "multiple "
            "threads",
            "[sequence_senders][single_thread_context][merge_each][merge][iterate]")
  {
    ...
    auto stop_after_on = [sched0, elapsed_ms](auto sched, auto after)
    {
      return schedule_after(sched, after)
           ...
           | STDEXEC::let_value(
               [elapsed_ms]() noexcept
               {
                 UNSCOPED_INFO("requesting stop  - at: " << std::setw(3) << elapsed_ms()
                                                         << "ms, on thread id: "
    ...
    auto error_after_on = [sched0, elapsed_ms](auto sched, auto after, auto error)
    {
      return schedule_after(sched, after)
           | STDEXEC::continues_on(sched0)  // serializes output on the sched0 strand
           | STDEXEC::let_value(
               [elapsed_ms, error]() noexcept
               {
                 UNSCOPED_INFO(error.what() << " - at: " << std::setw(3) << elapsed_ms()
    ...
    auto merged = sequences
                | flat_map([sched1](auto sequence)
                           { return sequence | delays_each_on(sched1, 10ms); });

    int count = 0;

    auto v = ex::sync_wait(
      ex::when_all(with_void(stop_after_on(sched1, 50ms)),
                   ignore_all_values(
                     merged | continues_each_on(sched0)  // serializes output on the sched0 strand
                     | then_each(
                       [&count, elapsed_ms](int x)
                       {
                         ++count;
                         UNSCOPED_INFO("item: " << x << ", arrived at: " << std::setw(3)
                                                << elapsed_ms() << "ms, on thread id: "
                                                << std::this_thread::get_id());
                         return count;
                       }))));
  ...
  TEST_CASE("merge_each - merge_each sender stops while merging all items from multiple threads",
            "[sequence_senders][single_thread_context][merge_each][merge][iterate]")
  {
    exec::single_thread_context ctx0;
    ex::scheduler auto          sched0 = ctx0.get_scheduler();
    ...
    auto v = ex::sync_wait(
      ex::when_all(with_void(stop_after_on(sched1, 40ms)),
                   ignore_all_values(
                     merged | continues_each_on(sched0)  // serializes output on the sched0 strand
                     | then_each(
                       [&count, elapsed_ms](int x)
