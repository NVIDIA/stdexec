...
namespace {
  ...
  TEST_CASE(
    ...
    [[maybe_unused]] std::array<empty_sequence_t, 2> array{
      empty_sequence(),
      empty_sequence()};

    [[maybe_unused]] auto sequences = iterate(std::views::all(array));
    using sequences_t = decltype(sequences);

    STATIC_REQUIRE(ex::__ok<
      item_types_of_t<sequences_t>>);
    STATIC_REQUIRE(ex::__ok<
      stdexec::completion_signatures_of_t<sequences_t>>);

    [[maybe_unused]] auto merged = merge_each(sequences);
    using merged_t = decltype(merged);


    STATIC_REQUIRE(ex::__ok<
      item_types_of_t<merged_t>>);
    STATIC_REQUIRE(ex::__ok<
      stdexec::completion_signatures_of_t<merged_t>>);
  ...
  TEST_CASE(
    ...
    [[maybe_unused]] std::array<range_sender_t, 2> array{
      range(100,120),
      range(200,220)};
  ...
  TEST_CASE(
    ...
    STATIC_REQUIRE_FALSE(std::same_as<
      item_types_of_t<range_sequence_t>,
      item_types<>>);
    STATIC_REQUIRE(__equivalent<
    ...
    STATIC_REQUIRE(__equivalent<
      ...
      ex::completion_signatures<
        ex::set_value_t(empty_sequence_t)>>);

    auto sequences = merge(
      ex::just(range(100, 120)),
