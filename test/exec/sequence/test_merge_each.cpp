...
namespace
{
  ...
  static constexpr auto flat_map = [](auto&& f)
  {
    ...
    return STDEXEC::__closure<decltype(map_merge), decltype(f)>{{static_cast<decltype(f)&&>(f)},
                                                                {},
                                                                {}};
  };
  ...
  TEST_CASE("merge_each - merge two sequence senders of integers",
            "[sequence_senders][merge_each][empty_sequence]")
  {
  ...
  TEST_CASE("merge_each - merge sequence of two sequence senders of integers and one empty "
            "sequence",
            "[sequence_senders][merge_each][merge][empty_sequence]")
  {
    ...
    STATIC_REQUIRE(__equivalent<ex::completion_signatures_of_t<just_range_sender_t>,
    ...
    STATIC_REQUIRE(__equivalent<ex::completion_signatures_of_t<just_empty_sender_t>,
                                ex::completion_signatures<ex::set_value_t(empty_sequence_t)>>);

    auto sequences    = merge(ex::just(range(100, 120)),
                           ex::just(empty_sequence()),
                           ex::just(range(200, 220)));
    using sequences_t = decltype(sequences);
