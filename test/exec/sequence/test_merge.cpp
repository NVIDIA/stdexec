...
namespace
{
  ...
  TEST_CASE("merge - merge two sequence senders of no elements",
  ...
  TEST_CASE("merge - merge three sequence senders of no elements",
            "[sequence_senders][merge][empty_sequence]")
  {
    int  counter = 0;
    auto merged  = exec::merge(exec::empty_sequence(),
                              exec::empty_sequence(),
                              exec::empty_sequence());
    auto op      = exec::subscribe(merged, null_receiver{});
    ex::start(op);
  ...
  TEST_CASE("merge - merge sender of 2 senders", "[sequence_senders][merge]")
  {
    ...
    auto transformed = exec::transform_each(merged,
                                            ex::then(
                                              [&value, &count](int x) noexcept
                                              {
                                                value += x;
                                                ++count;
                                              }));
  ...
  TEST_CASE("merge - merge sender of 2 senders and ignores all values",
  ...
  TEST_CASE("merge - merge sender merges all items", "[sequence_senders][merge][iterate]")
  {
    ...
    auto                       sum = exec::merge(range(100, 120), range(200, 220), range(300, 320))
             | then_each(
                 [&total, &count, &max](int x) noexcept
                 {
                   std::ptrdiff_t current = 0;
                   current                = std::abs(reinterpret_cast<char*>(&current)
                                      - reinterpret_cast<char*>(&max));
  ...
  TEST_CASE("merge - merge sender merges all items from multiple threads",
            "[sequence_senders][static_thread_pool][merge][iterate]")
  {
    ...
    int  total = 0;
    int  count = 0;
    auto sum   = exec::merge(range(100, 120) | continues_each_on(sched0),
             ...
             | then_each(
                 [&total, &count](auto v)
                 {
                   ...
                   total += x;
                   ++count;
                   UNSCOPED_INFO("item: " << x << ", from thread id: " << id
  ...
  TEST_CASE("merge - can be customized late", "[merge][ignore_all_values]")
  {
    ...
    auto adaptor        = ex::on(sched, ex::then([](std::string x) { return x + ", world"; }))
                 | with_scheduler;
    auto snd = exec::merge(start | exec::transform_each(adaptor),
