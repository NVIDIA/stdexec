...
namespace {
  ...
  TEST_CASE("test_scheduler - parse empty diagram", "[sequence_senders][test_scheduler][marbles]") {
    test_context __test{};
  ...
  TEST_CASE(
    ...
    CHECK(__clock.now() == test_clock::time_point{0ms});
    auto expected = get_marbles_from(__clock, "=^(0|)"_mstr);
  ...
  TEST_CASE(
    ...
    CHECK(__clock.now() == test_clock::time_point{0ms});
    auto __scheduler = __test.get_scheduler();
    auto __sequence = schedule_after(__scheduler, 2ms)
                    | stdexec::then([]() noexcept { return '0'; });
    auto expected = get_marbles_from(__clock, "=^--(0|)"_mstr);
    auto actual = __test.get_marbles_from(__sequence);
  ...
  TEST_CASE(
    ...
    auto __sequence = __test.get_marble_sequence_from("  -a--b---c|"_mstr);
    auto expected = get_marbles_from(__clock, "=^-a--b---c|"_mstr);
    auto actual = __test.get_marbles_from(__sequence);
  ...
  TEST_CASE(
    ...
    auto __clock = __test.get_clock();
    CHECK(test_clock::time_point{0ms} == __clock.now());
    auto __sequence = __test.get_marble_sequence_from("  012--|"_mstr);
  ...
  TEST_CASE(
    ...
    auto __sequence = __test.get_marble_sequence_from("   5s       0 5s 1 5s 2 100ms |"_mstr);
    auto expected = get_marbles_from(__clock, "=^ 5s 100ms 0 5s 1 5s 2       |"_mstr);

    auto actual =
      __test.get_marbles_from(__sequence | delays_each_on(__test.get_scheduler(), 100ms), 16s);
