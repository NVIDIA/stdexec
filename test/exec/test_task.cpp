...
namespace
{
  ...
  auto test_stickiness_for_two_single_thread_contexts_(auto scheduler1,
                                                       auto scheduler2,
                                                       auto id1,
                                                       auto id2) -> exec::task<void>
  {
    ...
    co_await test_stickiness_for_two_single_thread_contexts_nested(scheduler1,
                                                                   scheduler2,
                                                                   id1,
                                                                   id2);
  ...
  auto test_stickiness_for_two_single_thread_contexts_with_sender_(auto scheduler1,
                                                                   auto scheduler2,
                                                                   auto id1,
                                                                   auto id2) -> exec::task<void>
  {
  ...
  auto test_stickiness_for_two_single_thread_contexts(auto scheduler1,
                                                      auto scheduler2,
                                                      auto id1,
                                                      auto id2) -> exec::task<void>
  {
  ...
  auto test_stickiness_for_two_single_thread_contexts_with_sender(auto scheduler1,
                                                                  auto scheduler2,
                                                                  auto id1,
                                                                  auto id2) -> exec::task<void>
  {
    co_await exec::reschedule_coroutine_on(scheduler1);
    CHECK(get_id() == id1);
    co_await test_stickiness_for_two_single_thread_contexts_with_sender_(scheduler1,
                                                                         scheduler2,
                                                                         id1,
                                                                         id2);
    CHECK(get_id() == id1);
  ...
  TEST_CASE("Test stickiness with two single threads", "[types][sticky][task]")
  {
    ...
    ex::sync_wait(ex::when_all(ex::schedule(scheduler1) | ex::then([] { thread_id = 1; }),
            ...
            "[types][sticky][task]")
  {
    ...
    auto id1 = 1;
    auto id2 = 2;
    auto t   = ex::starts_on(scheduler1,
                           test_stickiness_for_two_single_thread_contexts_with_sender_(scheduler1,
                                                                                       scheduler2,
                                                                                       id1,
                                                                                       id2));
    ex::sync_wait(std::move(t) | ex::then([&] { CHECK(get_id() == id1); }));
  ...
  auto test_stick_on_main(ex::scheduler auto    sched1,
