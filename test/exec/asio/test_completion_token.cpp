...
namespace
{
  ...
  static_assert(noexcept(detail::completion_token::convert<int const &>(std::declval<int&>())));
  static_assert(noexcept(detail::completion_token::convert<int const>(std::declval<int>())));
  static_assert(
    noexcept(detail::completion_token::convert<std::string>(std::declval<std::string>())));
  static_assert(
    !noexcept(detail::completion_token::convert<std::string>(std::declval<std::string const &>())));
  static_assert(
    !noexcept(detail::completion_token::convert<std::string>(std::declval<char const *>())));
  ...
  class connect_shared_receiver
  {
    ...
    void complete_(Tag const & tag, Args&&... args) noexcept
    {
      CHECK(ptr_);
  ...
  TEST_CASE("Asio-based asynchronous operation ends with error when canceled",
            "[asioexec][completion_token]")
  {
    ...
    static_assert(set_equivalent<completion_signatures_of_t<decltype(sender), env<>>,
  ...
  decltype(auto) async_throw_from_initiation(CompletionToken&& token)
  {
    ...
    return asio_impl::async_initiate<CompletionToken, signature_type>(
      [](auto const &) { throw std::logic_error("Test"); },
      token);
  }

  TEST_CASE("When initiating the asynchronous operation throws this causes the "
            "sender to send a std::exception_ptr",
            "[asioexec][completion_token]")
  {
    std::exception_ptr ex;
    auto               op = connect_shared(async_throw_from_initiation(completion_token),
                             expect_error_receiver_ex(ex));
    CHECK(!ex);
    start_shared(std::move(op));
    CHECK(ex);
  }
  ...
  decltype(auto) async_throw_from_completion(Executor const & ex, CompletionToken&& token)
  {
    using signature_type = void();
    return asio_impl::async_initiate<CompletionToken, signature_type>(
      [ex](auto h)
      {
        auto const assoc = asio_impl::get_associated_executor(h, ex);
        asio_impl::post(ex,
  ...
  TEST_CASE("When a completion handler invoked as part of a composed "
            ...
            "[asioexec][completion_token]")
  {
    ...
    using sender_type          = decltype(std::declval<asio_impl::system_timer&>().async_wait(
      completion_token));
  ...
  template <typename Executor, typename CompletionToken>
  decltype(auto) async_indirect_completion_handler(Executor const &       ex,
  ...
  TEST_CASE("If the completion handler outlives completion of the operation "
            ...
            "[asioexec][completion_token]")
  {
    ...
    std::shared_ptr<void> ptr;
    asio_impl::io_context ctx;
    auto sender = async_indirect_completion_handler_throw_from_completion(ctx.get_executor(),
                                                                          ptr,
                                                                          completion_token);
    CHECK(!ctx.poll());
    CHECK(ctx.stopped());
  ...
  decltype(auto) async_single_shot(CompletionToken&& token)
  {
    using signature_type = void();
    return asio_impl::async_initiate<CompletionToken, signature_type>(
      [ptr = std::make_unique<int>(5)](auto&& h) { std::forward<decltype(h)>(h)(); },
      token);
  }
  ...
  TEST_CASE("When an operation is abandoned by the initiating function "
  ...
  TEST_CASE("Intermediate completion handlers being passed between threads has "
            ...
            "[asioexec][completion_token]")
  {
    ...
    auto const initiating_function = [&](auto&& token)
    {
      return asio_impl::async_initiate<decltype(token), void()>(
        [&a, &b](auto&& h)
        {
          ping_pong<std::remove_cvref_t<decltype(h)>>{a.context(),
                                                      b.context(),
  ...
  TEST_CASE("When there are two parallel paths of asynchronous execution, and "
        ...
        return asio_impl::async_initiate<decltype(token), void()>(
          [&](auto&& h)
          {
            auto ptr = std::make_shared<std::remove_cvref_t<decltype(h)>>(
              std::forward<decltype(h)>(h));
            auto const ex = asio_impl::get_associated_executor(*ptr, ctx.get_executor());
            if (u)
            {
  ...
  TEST_CASE("When the Asio signature suggests the operation will send an "
            ...
            "[asioexec][completion_token]")
  {
    class obj
    {
     ...
     public:
      ...
      bool operator==(obj const & rhs) const noexcept
      {
    ...
    obj const          expected;
    std::exception_ptr ex;
    auto const         initiating_function = [](obj o, auto&& token)
    {
    ...
    auto a = connect_shared(initiating_function(obj{}, completion_token),
                            expect_value_receiver(expected));
    auto b = connect_shared(initiating_function(obj(true), completion_token),
                            expect_error_receiver_ex(ex));
    start_shared(std::move(a));
    CHECK(!ex);
    start_shared(std::move(b));
  ...
  struct value_category_receiver
  {
    ...
    void set_value(std::mutex const &) && noexcept
    {
      CHECK(kind_ == kind::none);
  ...
  TEST_CASE("When the operation declares separate rvalue and lvalue completion signatures they are "
            "appropriately passed through",
            "[asioexec][completion_token]")
  {
    auto const initiating_function = [](bool const rvalue, auto&& token)
    {
    ...
    auto rvalue = connect_shared(initiating_function(true, completion_token),
                                 value_category_receiver(rvalue_kind));
    auto lvalue = connect_shared(initiating_function(false, completion_token),
                                 value_category_receiver(lvalue_kind));
  ...
  TEST_CASE("When the operation declares separate rvalue and const lvalue completion signatures "
            "they are "
            ...
            "[asioexec][completion_token]")
  {
    auto const initiating_function = [](bool const rvalue, auto&& token)
    {
      return asio_impl::async_initiate<decltype(token),
                                       void(std::mutex&&),
                                       void(std::mutex const &)>(
        [rvalue](auto&& h)
        {
          std::mutex m;
          if (rvalue)
  ...
  TEST_CASE("Lvalue invocation of the completion handler is supported (for compatibility with "
            "Boost.Beast "
  ...
  TEST_CASE("Cancellation is delivered thread safely even when emitted thread unsafely",
            "[asioexec][use_sender]")
  {
    ...
    auto const initiating_function = [&]<typename CompletionToken>(CompletionToken&& token)
    {
      return asio_impl::async_initiate<CompletionToken, void()>(
        [&](auto h)
        {
          asio_impl::get_associated_cancellation_slot(h).assign(
            [&](auto&&...) noexcept
            {
              std::unique_lock const l(m, std::try_to_lock);
              thread_safe = bool(l);
              cancelled   = true;
            });
          auto const ex = asio_impl::get_associated_executor(h, ctx.get_executor());
          asio_impl::post(ex,
    ...
    {
      auto        ptr = connect_shared(std::move(sender),
  ...
  TEST_CASE("Upon completion the stop token is no longer in use", "[asioexec][use_sender]")
  {
    ...
    auto sender = initiating_function(completion_token);
    auto op     = ::STDEXEC::connect(std::move(sender),
  ...
  TEST_CASE("Upon abandonment within the initiating function the stop token is no longer in use",
            "[asioexec][use_sender]")
  {
    bool                           cancelled = false;
    ::STDEXEC::inplace_stop_source source;
  ...
  TEST_CASE("Upon exception from the initiating function the stop token is no longer in use",
            "[asioexec][use_sender]")
  {
    bool                           cancelled = false;
    std::exception_ptr             ex;
  ...
  TEST_CASE("When the initiating function starts an operation and then throws an exception the "
            "operation "
            ...
            "[asioexec][use_sender]")
  {
    ...
    auto const initiating_function = [&]<typename CompletionToken>(CompletionToken&& token)
    {
      return asio_impl::async_initiate<CompletionToken, void()>(
        ...
        {
          t.expires_after(std::chrono::years(1));
          auto const ex   = asio_impl::get_associated_executor(h, t.get_executor());
          auto const slot = asio_impl::get_associated_cancellation_slot(h);
          t.async_wait(asio_impl::bind_executor(
            ex,
            asio_impl::bind_cancellation_slot(slot,
            ...
            "[asioexec][completion_token]")
  {
    ...
    auto op = ::STDEXEC::connect(t.async_wait() | ::STDEXEC::then([](auto ec) { CHECK(!ec); }),
  ...
  TEST_CASE("Substitution into async_result<completion_token, ...>::initiate is SFINAE-friendly",
            "[asioexec][completion_token]")
  {
    asio_impl::io_context      ctx;
    ...
    auto op     = ::STDEXEC::connect(std::move(sender)
                                   | ::STDEXEC::then(
                                     [](auto const ec, auto const bytes_transferred)
                                     {
                                       CHECK(ec);
  ...
  TEST_CASE("Asio operations which declare completion by value but send a mutable lvalue work",
            "[asioexec][completion_token]")
  {
    auto const initiating_function = [](auto&& token)
    {
