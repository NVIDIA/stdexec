...
TEST_CASE("Implementation detail sender factory works for a single async object", "[lifetime]") {
  std::size_t n{};
  state s{n};
  std::tuple<::exec::detail::lifetime::storage_for_object<int>> storage;
  auto sender = ::exec::detail::lifetime::make_sender(
    std::tuple(
      [&](int& i) {
        CHECK(i == 5);
        return ::STDEXEC::just();
