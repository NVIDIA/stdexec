...
TEST_CASE("transfer can be customized", "[adaptors][transfer]") {
  ...
  val_type1 res{0};
  auto op = ex::connect(std::move(snd), expect_value_receiver_ex<val_type1>(&res), empty_env{});
  ex::start(op);
  REQUIRE(res.val_ == 53);
}
...
TEST_CASE("transfer follows schedule_from customization", "[adaptors][transfer]") {
  ...
  val_type2 res{0};
  auto op = ex::connect(std::move(snd), expect_value_receiver_ex<val_type2>(&res), empty_env{});
  ex::start(op);
  REQUIRE(res.val_ == 59);
}
...
TEST_CASE("transfer can be customized with two schedulers", "[adaptors][transfer]") {
  ...
  val_type3 res{0};
  auto op = ex::connect(std::move(snd), expect_value_receiver_ex<val_type3>(&res), empty_env{});
  ex::start(op);
