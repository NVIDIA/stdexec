...
namespace {
  ...
  TEST_CASE(
    "task_scheduler starts work on the correct execution context",
  ...
  TEST_CASE(
    "bulk_unchunked dispatches correctly through task_scheduler",
    "[scheduler][task_scheduler]") {
    ex::task_scheduler sched{dummy_scheduler<test_domain>{}};
    auto sndr = ex::on(
      sched, ex::just(-1) | ex::bulk_chunked(ex::par_unseq, 100, [](size_t, size_t, int&) { }));
    g_called = false;
  ...
  TEST_CASE(
    "task_scheduler cannot be constructed from a parallel_scheduler",
    "[scheduler][task_scheduler]") {
