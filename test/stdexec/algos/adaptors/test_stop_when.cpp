...
namespace
{
  ...
  TEST_CASE("stop-when substitutes its token when the receiver's token is unstoppable",
            "[adaptors][stop-when]")
  {
  ...
  {
    ex::inplace_stop_source source;
    wait_for_value(ex::__stop_when(ex::__stop_when(isTokenStoppable(), source.get_token()),
                                   source.get_token()),
                   true);
  }
  ...
  ex::sender auto make_stop_callback(Fn&& fn) noexcept
  {
    return ex::read_env(ex::get_stop_token)
         | ex::then(
             [fn = std::forward<Fn>(fn)](auto token) mutable noexcept
             {
  ...
  TEST_CASE("callbacks registered with stop-when's token can be invoked", "[adaptors][stop-when]")
  {
...

    {
      ...
      wait_for_value(ex::__stop_when(snd, source.get_token())
                       | ex::then(
                         [&](auto&& optCallback) noexcept
                         {
                           source.request_stop();
...

    {
      ...
      wait_for_value(ex::__stop_when(snd, source2.get_token())
                       | ex::then(
                         [&](auto&& optCallback) noexcept
                         {
