...
namespace
{
  ...
  TEST_CASE("then forwards env", "[adaptors][then]")
  {
    ...
    {
      auto snd = just_with_env<value_env const &, int>{.env_ = value_env{100}, .values_ = {0}}
               | ex::then([](int) {});
      CHECK(value_query(ex::get_env(snd)) == 100);
    }
  }
  ...
  TEST_CASE("then has the values_type corresponding to the given values", "[adaptors][then]")
  {
    ...
    check_val_types<ex::__mset<pack<std::string>>>(ex::just()
  ...
  TEST_CASE("then keeps error_types from input sender", "[adaptors][then]")
  {
    ...
    check_err_types<ex::__mset<std::exception_ptr>>(ex::just() | ex::continues_on(sched2)
                                                    | ex::then([]() noexcept {}));
    check_err_types<ex::__mset<std::exception_ptr, int>>(ex::just() | ex::continues_on(sched3)
                                                         | ex::then([] {}));
  }
