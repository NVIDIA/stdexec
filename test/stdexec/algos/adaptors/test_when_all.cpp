...
namespace
{
  ...
  TEST_CASE("when_all completes when children complete", "[adaptors][when_all]")
  {
    ...
    bool              called{false};
    ex::sender auto   snd = ex::when_all(ex::just(11) | ex::continues_on(sched),
  ...
  TEST_CASE("when_all terminates with error if one child terminates with error",
            "[adaptors][when_all]")
  {
  ...
  TEST_CASE("when_all cancels remaining children if error is detected", "[adaptors][when_all]")
  {
    ...
    ex::sender auto   snd =
      ex::when_all(ex::starts_on(sched, ex::just()) | ex::then([&] { called1 = true; }),
                   ex::starts_on(sched, ex::just(5)) | ex::continues_on(err_sched),
                   ex::starts_on(sched, ex::just()) | ex::then([&] { called3 = true; })
                     | ex::let_stopped(
                       [&]
                       {
                         cancelled = true;
  ...
  TEST_CASE("when_all propagates domain from children", "[adaptors][when_all]")
  {
    ...
    auto snd    = ex::when_all(ex::starts_on(sched1, ex::just(13)),
  ...
  TEST_CASE("when_all_with_variant works with custom domain", "[adaptors][when_all]")
  {
    ...
    SECTION("late customization")
    {
      ...
      auto snd = ex::starts_on(scheduler(),
                               ex::when_all_with_variant(ex::just(3), ex::just(0.1415)));
      wait_for_value(std::move(snd), std::string{"hello world"});
    }
  }
