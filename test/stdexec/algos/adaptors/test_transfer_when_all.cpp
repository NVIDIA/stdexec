...
namespace
{
  ...
  TEST_CASE("transfer_when_all with no senders transfers the result",
            "[adaptors][transfer_when_all]")
  {
    impulse_scheduler sched;
  ...
  {
    auto snd = ex::transfer_when_all_with_variant(inline_scheduler{},
                                                  ex::just(3),
                                                  ex::just(0.1415));
    static_assert(ex::sender<decltype(snd)>);
    (void) snd;
  ...
  {
    ex::sender auto snd = ex::transfer_when_all_with_variant(inline_scheduler{},
                                                             ex::just(2),
                                                             ex::just(3.14));
  ...
  TEST_CASE("transfer_when_all_with_variant works with custom domain",
            "[adaptors][transfer_when_all]")
  {
    constexpr auto hello = []
    {
    ...
    SECTION("late customization")
    {
      ...
      auto snd = ex::starts_on(scheduler(),
                               ex::transfer_when_all_with_variant(inline_scheduler(),
                                                                  ex::just(3),
                                                                  ex::just(0.1415)));
  ...
  TEST_CASE("transfer_when_all_with_variant finds transfer_when_all customizations",
            "[adaptors][transfer_when_all]")
  {
    constexpr auto hello = []
    {
