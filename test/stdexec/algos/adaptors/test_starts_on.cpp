...
namespace
{
  ...
  TEST_CASE("starts_on calls the given sender when the scheduler dictates", "[adaptors][starts_on]")
  {
    bool called{false};
  ...
  TEST_CASE("starts_on has the values_type corresponding to the given values",
            "[adaptors][starts_on]")
  {
    inline_scheduler sched{};
  ...
  struct move_checker
  {
    ...
    move_checker(move_checker const & other) noexcept
    {
      REQUIRE(other.valid);
      valid = true;
    }

    auto operator=(move_checker const & other) noexcept -> move_checker&
    {
      REQUIRE(other.valid);
      valid = true;
      return *this;
  ...
  struct move_checking_inline_scheduler
  {
    ...
    auto operator==(move_checking_inline_scheduler const &) const noexcept -> bool
    {
      return true;
    }
