...
namespace
{
  ...
  constexpr auto test_constexpr() noexcept
  {
    ...
    int  i  = 0;
    auto op = ex::connect(ex::just(666)
                            | ex::bulk(ex::par,
                                       42,
                                       [](std::size_t item, int& val) noexcept
                                       { val += static_cast<int>(item); }),
                          receiver{i});
    ex::start(op);
    return i;
  }
  ...
  TEST_CASE("bulk keeps values_type from input sender", "[adaptors][bulk]")
  {
    ...
    check_val_types<ex::__mset<pack<double>>>(ex::just(4.2)
                                              | ex::bulk(ex::par, n, [](int, double) {}));
  ...
  TEST_CASE("bulk_unchunked keeps values_type from input sender", "[adaptors][bulk]")
  {
    ...
    check_val_types<ex::__mset<pack<double>>>(ex::just(4.2)
                                              | ex::bulk_unchunked(ex::par, n, [](int, double) {}));
    check_val_types<ex::__mset<pack<double, std::string>>>(
      ex::just(4.2, std::string{})
...
#  endif
  ...
  TEST_CASE("bulk_chunked keeps error_types from input sender", "[adaptors][bulk]")
  {
    ...
    check_err_types<ex::__mset<std::exception_ptr>>(
      ...
      | ex::bulk_chunked(ex::par, n, [](int, int) noexcept {}));
    check_err_types<ex::__mset<int>>(ex::just_error(n)
                                     | ex::bulk_chunked(ex::par, n, [](int, int) noexcept {}));
    check_err_types<ex::__mset<int>>(ex::just() | ex::continues_on(sched3)
                                     | ex::bulk_chunked(ex::par, n, [](int, int) noexcept {}));
#if !STDEXEC_NO_STDCPP_EXCEPTIONS()
  ...
  TEST_CASE("bulk_unchunked cannot be used to change the value type", "[adaptors][bulk]")
  {
    ...
    auto snd = ex::just(magic_number)
             | ex::bulk_unchunked(ex::par,
                                  n,
                                  [](int, int) { return function_object_t<int>{nullptr}; });
  ...
  TEST_CASE("bulk works with static thread pool", "[adaptors][bulk]")
  {
    ...
    SECTION("Without values in the set_value channel")
    {
      for (std::size_t n = 0; n < 9u; n++)
      {
        ...
        std::size_t const actual = static_cast<std::size_t>(
          std::count(counter.begin(), counter.end(), 1));
        std::size_t const expected = n;
    ...
    SECTION("With values in the set_value channel that can be taken by reference")
    {
      for (std::size_t n = 0; n < 9; n++)
      {
        ...
        auto snd = ex::just(std::move(vals)) | ex::continues_on(sch)
                 ...
                 | ex::bulk(ex::par,
                            n,
                            [](std::size_t idx, std::vector<int>& vals) { ++vals[idx]; });
  ...
  TEST_CASE("bulk_chunked works with static thread pool", "[adaptors][bulk]")
  {
    ...
    SECTION("With exception")
    {
      ...
      auto          snd = ex::just() | ex::continues_on(sch)
               | ex::bulk_chunked(ex::par,
                                  n,
                                  [](int, int) { throw std::runtime_error("bulk_chunked"); });
    ...
    SECTION("With concurrent enqueueing")
    {
      ...
      ex::sender auto snd = ex::when_all(ex::schedule(sch)
                                           | ex::bulk_chunked(ex::par,
                                                              n,
  ...
  TEST_CASE("bulk_unchunked works with static thread pool", "[adaptors][bulk]")
  {
    ...
    SECTION("With exception")
    {
      ...
      auto       snd = ex::just() | ex::continues_on(sch)
               | ex::bulk_unchunked(ex::par,
                                    n,
                                    [](int) { throw std::runtime_error("bulk_unchunked"); });
  ...
  TEST_CASE("eager customization of bulk works with static thread pool", "[adaptors][bulk]")
  {
    ...
    SECTION("Without values in the set_value channel")
    {
      ...
      auto const actual = static_cast<std::size_t>(std::count(tids.begin(), tids.end(), tids[0]));
      std::size_t const wrong = tids.size();

      CHECK(actual != wrong);
    }
  }

  TEST_CASE("eager customization of bulk_chunked works with static thread pool", "[adaptors][bulk]")
  {
    exec::static_thread_pool pool{4};
    ex::scheduler auto       sch = pool.get_scheduler();
  ...
  int number_of_threads_in_bulk(Sched sch, Policy const & policy, int n)
  {
  ...
  TEST_CASE("static thread pool execute bulk work in accordance with the execution policy",
            "[adaptors][bulk]")
  {
    exec::static_thread_pool pool{4};
    ex::scheduler auto       sch = pool.get_scheduler();
  ...
  template <class Sched, class Policy>
  int number_of_threads_in_bulk_chunked(Sched sch, Policy const & policy, int n)
  {
    std::vector<std::thread::id> tids(n);
    auto                         fun = [&tids](std::size_t b, std::size_t e)
    {
  ...
  TEST_CASE("static thread pool execute bulk_chunked work in accordance with the execution policy",
            "[adaptors][bulk]")
  {
    exec::static_thread_pool pool{4};
    ex::scheduler auto       sch = pool.get_scheduler();
