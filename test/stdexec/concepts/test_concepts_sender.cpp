...
namespace
{
  ...
  TEST_CASE("sender_to concept does not instantiate the receiver's completion functions",
            "[concepts][sender]")
  {
  ...
  TEST_CASE("can query completion signatures for a typed sender that sends nothing",
            "[concepts][sender]")
  {
  ...
  TEST_CASE("can query completion signatures for a typed sender that sends int",
            "[concepts][sender]")
  {
  ...
  {
    using sender_concept        = STDEXEC::sender_t;
    using completion_signatures = ex::completion_signatures<ex::set_value_t(int, double),
  ...
  TEST_CASE("check completion signatures for sender that also supports error codes",
            "[concepts][sender]")
  {
  ...
  TEST_CASE("r5 sender emits deprecated diagnostics", "[concepts][sender]")
  {
    ...
    static_assert(
      std::same_as<decltype(ex::get_completion_signatures(my_r5_sender0{}, ex::env<>{})),
