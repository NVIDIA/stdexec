...
namespace
{
  ...
  SCOPE_TEST_CASE("the scope type has a static size_t named max_associations with a positive value")
  {
    STATIC_REQUIRE(std::same_as<std::size_t const, decltype(TestType::max_associations)>);
  ...
  SCOPE_TEST_CASE("deferred join-senders work")
  {
    ...
    wait_for_value(ex::when_all(ex::just()
                                  | ex::let_value(
                                    [&]() noexcept
                                    {
  ...
  SCOPE_TEST_CASE("closed scopes refuse new associations")
  {
    ...
    ex::sync_wait(ex::associate(ex::just(&firstSenderRan), scope.get_token())
  ...
  SCOPE_TEST_CASE("spawn lots of concurrent join-senders looking for UB")
  {
    ...
    for (int i = 0; i < 100; ++i)
    {
      ex::spawn(ex::starts_on(sched, innerScope.join())
                  | ex::upon_error(
                    [&](auto error) noexcept
                    {
  ...
  TEST_CASE("counting_scope::request_stop signals associated senders", "[types][counting_scope]")
  {
    ...
    wait_for_value(ex::associate(ex::read_env(ex::get_stop_token), scope.get_token())
