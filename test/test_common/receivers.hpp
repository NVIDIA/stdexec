...
namespace
{
  ...
  struct expect_void_receiver_ex
  {
    ...
    template <class... Ty>
    void set_value(Ty const &...) noexcept
    {
      *executed_ = true;
    }
  ...
  inline auto
  to_comparable(std::exception_ptr eptr) -> std::pair<std::type_info const &, std::string>
  {
    STDEXEC_TRY
    {
  ...
  template <class T>
  inline auto to_comparable(T const & value) -> T const &
  {
    return value;  // NOLINT
  }
  ...
  struct typecat_receiver
  {
    ...
    void set_value(T const & v) noexcept
    {
      *value_ = v;
      *cat_   = typecat::cref;
    }
