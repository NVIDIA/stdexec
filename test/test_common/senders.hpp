...
namespace
{
  ...
  struct fallible_just
  {
    ...
    struct operation : immovable
    {
      ...
      void start() & noexcept
      {
        std::apply([&](Values&... ts) { ex::set_value(std::move(rcvr_), std::move(ts)...); },
                   values_);
      }
    };
    ...
    {
      return ex::completion_signatures<ex::set_value_t(Values...),
    ...
    struct operation
    {
      ...
      struct on_stopped
      {
        ...
        void operator()() noexcept
        {
          ...
          while (!self_.state_.compare_exchange_weak(expected,
                                                     state_t::stopped,
  ...
  struct non_default_constructible
  {
    ...
    friend auto
    operator==(non_default_constructible const & lhs, non_default_constructible const & rhs) -> bool
    {
      return lhs.x == rhs.x;
    }
  };
  ...
  struct succeed_n_sender
  {
    ...
    struct operation
    {
      void start() noexcept
      {
        if (--*counter_ == -1)
        {
          std::apply([&](Args&... args) -> void { Tag{}(std::move(rcvr_), std::move(args)...); },
                     args_);
        }
        else
        {
