...
namespace Catch {
  ...
  struct StringMaker<std::chrono::time_point<_Clock, _Duration>> {
    static std::string convert(const std::chrono::time_point<_Clock, _Duration>& __at) {
      return std::to_string(std::chrono::duration_cast<std::chrono::milliseconds>(__at.time_since_epoch()).count()) + "ms";
    }
  };
  ...
  struct StringMaker<std::chrono::duration<_Rep, _Period>> {
    static std::string convert(const std::chrono::duration<_Rep, _Period>& __duration) {
      return std::to_string(std::chrono::duration_cast<std::chrono::milliseconds>(__duration).count()) + "ms";
    }
  };
...
namespace {
  ...
  [[maybe_unused]] static constexpr auto log_start = [](auto sequence, auto message) {
  ...
  [[maybe_unused]] static constexpr auto log_sequence = [](auto sequence, auto message) {
    return sequence
  ...
  [[maybe_unused]] static constexpr auto emits_stopped = []() {
    return ex::just()
  ...
  [[maybe_unused]] static constexpr auto emits_error = [](auto error) {
    return ex::just()
