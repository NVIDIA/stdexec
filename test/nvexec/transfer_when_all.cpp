...
namespace
{
  ...
  TEST_CASE("nvexec transfer_when_all returns a sender",
  ...
  TEST_CASE("nvexec transfer_when_all with environment returns a sender",
            "[cuda][stream][adaptors][transfer_when_all]")
  {
    nvexec::stream_context stream_ctx{};
    auto                   gpu = stream_ctx.get_scheduler();
  ...
  TEST_CASE("nvexec transfer_when_all with no senders",
            "[cuda][stream][adaptors][transfer_when_all]")
  {
    nvexec::stream_context stream_ctx{};
    auto                   gpu = stream_ctx.get_scheduler();
  ...
  TEST_CASE("nvexec transfer_when_all two senders on same scheduler",
            "[cuda][stream][adaptors][transfer_when_all]")
  {
    nvexec::stream_context stream_ctx{};
    auto                   gpu  = stream_ctx.get_scheduler();
    auto                   snd1 = ex::transfer_when_all(gpu,
                                      ex::just(3) | ex::continues_on(gpu),
                                      ex::just(0.1415) | ex::continues_on(gpu));
    auto                   snd2 = std::move(snd1) | ex::then([](int x, double y) { return x + y; });
    wait_for_value(std::move(snd2), 3.1415);  // NOLINT(modernize-use-std-numbers)
  }

  TEST_CASE("nvexec transfer_when_all_with_variant returns a sender",
  ...
  TEST_CASE("nvexec transfer_when_all_with_variant with environment returns a sender",
            "[cuda][stream][adaptors][transfer_when_all_with_variant]")
  {
    nvexec::stream_context stream_ctx{};
    auto                   gpu = stream_ctx.get_scheduler();
  ...
  TEST_CASE("nvexec transfer_when_all_with_variant two senders",
            "[cuda][stream][adaptors][transfer_when_all_with_variant]")
  {
    nvexec::stream_context stream_ctx{};
    auto                   gpu = stream_ctx.get_scheduler();
  ...
  TEST_CASE("nvexec transfer_when_all_with_variant basic example",
            "[cuda][stream][adaptors][transfer_when_all_with_variant]")
  {
    nvexec::stream_context stream_ctx{};
    auto                   gpu = stream_ctx.get_scheduler();
