...
struct recv_value {
  ...
  friend void tag_invoke(set_stopped_t, recv_value self) { *self.target_ = INT_MAX; }
};
struct recv_rvalref {
  int* target_;
  ...
  friend void tag_invoke(set_stopped_t, recv_rvalref&& self) { *self.target_ = INT_MAX; }
};
struct recv_ref {
  int* target_;
  ...
  friend void tag_invoke(set_stopped_t, recv_ref& self) { *self.target_ = INT_MAX; }
};
struct recv_cref {
  int* target_;
  ...
  friend void tag_invoke(set_stopped_t, const recv_cref& self) { *self.target_ = INT_MAX; }
};
...
TEST_CASE("can call set_value on a receiver with plain value type", "[cpo][cpo_receiver]") {
  static_assert(
      std::invocable<set_value_t, recv_value, int>, "cannot call set_value on recv_value");
  int val = 0;
...
TEST_CASE("can call set_value on a receiver with r-value ref type", "[cpo][cpo_receiver]") {
  static_assert(
      std::invocable<set_value_t, recv_rvalref, int>, "cannot call set_value on recv_rvalref");
  int val = 0;
  ex::set_value(recv_rvalref{&val}, 10);
  REQUIRE(val == 10);
}
TEST_CASE("can call set_value on a receiver with ref type", "[cpo][cpo_receiver]") {
  static_assert(std::invocable<set_value_t, recv_ref&, int>, "cannot call set_value on recv_ref");
  int val = 0;
  recv_ref recv{&val};
...
TEST_CASE("can call set_value on a receiver with const ref type", "[cpo][cpo_receiver]") {
  static_assert(std::invocable<set_value_t, recv_cref, int>, "cannot call set_value on recv_cref");
  int val = 0;
...
TEST_CASE("can call set_error on a receiver with plain value type", "[cpo][cpo_receiver]") {
  static_assert(
      std::invocable<set_error_t, recv_value, int>, "cannot call set_error on recv_value");
  int val = 0;
...
TEST_CASE("can call set_error on a receiver with r-value ref type", "[cpo][cpo_receiver]") {
  static_assert(
      std::invocable<set_error_t, recv_rvalref, int>, "cannot call set_error on recv_rvalref");
  int val = 0;
  ex::set_error(recv_rvalref{&val}, 10);
  REQUIRE(val == -10);
}
TEST_CASE("can call set_error on a receiver with ref type", "[cpo][cpo_receiver]") {
  static_assert(std::invocable<set_error_t, recv_ref&, int>, "cannot call set_error on recv_ref");
  int val = 0;
  recv_ref recv{&val};
...
TEST_CASE("can call set_error on a receiver with const ref type", "[cpo][cpo_receiver]") {
  static_assert(std::invocable<set_error_t, recv_cref, int>, "cannot call set_error on recv_cref");
  int val = 0;
...
TEST_CASE("can call set_stopped on a receiver with plain value type", "[cpo][cpo_receiver]") {
  static_assert(std::invocable<set_stopped_t, recv_value>, "cannot call set_stopped on recv_value");
  int val = 0;
...
TEST_CASE("can call set_stopped on a receiver with r-value ref type", "[cpo][cpo_receiver]") {
  static_assert(
      std::invocable<set_stopped_t, recv_rvalref>, "cannot call set_stopped on recv_rvalref");
  int val = 0;
  ex::set_stopped(recv_rvalref{&val});
  REQUIRE(val == INT_MAX);
}
TEST_CASE("can call set_stopped on a receiver with ref type", "[cpo][cpo_receiver]") {
  static_assert(std::invocable<set_stopped_t, recv_ref&>, "cannot call set_stopped on recv_ref");
  int val = 0;
  recv_ref recv{&val};
  ex::set_stopped(recv);
  REQUIRE(val == INT_MAX);
...
TEST_CASE("can call set_stopped on a receiver with const ref type", "[cpo][cpo_receiver]") {
  static_assert(std::invocable<set_stopped_t, recv_cref>, "cannot call set_stopped on recv_cref");
  int val = 0;
